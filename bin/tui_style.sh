...
_tui.theme_decl() {   # CLASS KEY VALUE
    ...
    if [[ -z "${_TP_SEEN[$cls]:-}" ]]; then _TP_SEEN[$cls]=1; _TP_ORDER+=("$cls"); fi
    case "$key" in
        fg)   _TP_FG[$cls]="$val" ;;
        bg)   _TP_BG[$cls]="$val" ;;
...
_tui._find_theme_collisions() {
    ...
    for base in "${!bases[@]}"; do
        _tui._theme_state_triplet "$base" title; title="$_TT"
        _tui._theme_triplet_set "$title" || continue

        _tui._theme_state_triplet "$base" focus; focus="$_TT"
        if _tui._theme_triplet_set "$focus"; then
            ring="$focus"
        else
            _tui._theme_state_triplet "$base" border; ring="$_TT"
        fi
