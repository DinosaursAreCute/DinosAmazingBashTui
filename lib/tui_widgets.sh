...
_tui_wx.new() {
    local id="$1" type="$2"
    if [[ -z "$id" || -z "$3" ]]; then echo "tui.$type: missing id or pane, skipping widget" >&2; return 1; fi
    _TUI_W_TYPE[$id]="$type"; _TUI_W_PANE[$id]="$3"; _TUI_W_ROW[$id]="$4"
    _TUI_W_LABEL[$id]=""; _TUI_W_VALUE[$id]=""; _TUI_W_ACTION[$id]=""
    _TUI_W_ORDER+=("$id")
...
tui.password() {
    ...
    _TUI_W_PH[$1]="${4:-}"; _TUI_W_LABEL[$1]="${5:-}"; _TUI_W_SUBMIT[$1]="${6:-}"
}
tui.textarea() {
    _tui_wx.new "$1" textarea "$2" "$3" 1 || return 1
    _TUI_W_PH[$1]="${4:-}"; _TUI_W_ROWSPAN[$1]="${5:-0}"; _TUI_W_SUBMIT[$1]="${6:-}"
}
tui.list() {
    _tui_wx.new "$1" list "$2" "$3" 1 || return 1
    _TUI_W_ACTION[$1]="${4:-}"; _TUI_W_ROWSPAN[$1]="${5:-0}"; _WXSEL[$1]=-1; _WXTOP[$1]=0
    _tui_wx.arr "$1"; _WXA=()
}
tui.table() {
    _tui_wx.new "$1" table "$2" "$3" 1 || return 1
    _TUI_W_ACTION[$1]="${4:-}"; _TUI_W_ROWSPAN[$1]="${5:-0}"; _WXSEL[$1]=-1; _WXTOP[$1]=0; _WXCOLS[$1]=""
    _tui_wx.arr "$1"; _WXA=()
}
tui.select() {
    _tui_wx.new "$1" select "$2" "$3" 1 || return 1
    _TUI_W_LABEL[$1]="${4:-}"; _TUI_W_ACTION[$1]="${5:-}"; _WXSEL[$1]=-1
    _tui_wx.arr "$1"; _WXA=()
}
tui.progress() {
    _tui_wx.new "$1" progress "$2" "$3" 0 || return 1
    _TUI_W_LABEL[$1]="${4:-}"; _TUI_W_VALUE[$1]=0
}
...
# ── data setters ─────────────────────────────────────────────────────────
_tui_wx.redraw() { (( _TUI_RUNNING )) && _tui._draw_widget "$1"; return 0; }
_tui_wx.changed() { local fn="${_TUI_W_CHANGE[$1]:-}"; [[ -n "$fn" ]] && "$fn" "$1"; return 0; }

tui.list.set()   { local id="$1"; shift; _tui_wx.arr "$id"; _WXA=("$@"); (( ${#_WXA[@]} )) && _WXSEL[$id]=0 || _WXSEL[$id]=-1; _WXTOP[$id]=0; _tui_wx.redraw "$id"; }
tui.list.add()   { local id="$1"; shift; _tui_wx.arr "$id"; _WXA+=("$@"); (( ${_WXSEL[$id]:--1} < 0 )) && _WXSEL[$id]=0; _tui_wx.redraw "$id"; }
tui.list.clear() { _tui_wx.arr "$1"; _WXA=(); _WXSEL[$1]=-1; _WXTOP[$1]=0; _tui_wx.redraw "$1"; }
tui.list.count() { _tui_wx.arr "$1"; printf '%s\n' "${#_WXA[@]}"; }
tui.list.selected() { printf '%s\n' "${_WXSEL[$1]:--1}"; }
tui.list.item()  { _tui_wx.arr "$1"; local i="${2:-${_WXSEL[$1]:--1}}"; (( i >= 0 && i < ${#_WXA[@]} )) && printf '%s\n' "${_WXA[i]}"; return 0; }
tui.list.select() {
    _tui_wx.arr "$1"; local n=${#_WXA[@]} i=$2
    (( i >= n )) && i=$(( n - 1 )); (( i < 0 )) && i=-1
    _WXSEL[$1]=$i; _tui_wx.redraw "$1"
}
tui.table.set()   { local id="$1" head="$2"; shift 2; _tui_wx.arr "$id"; _WXCOLS[$id]="$head"; _WXA=("$@"); (( ${#_WXA[@]} )) && _WXSEL[$id]=0 || _WXSEL[$id]=-1; _WXTOP[$id]=0; _tui_wx.redraw "$id"; }
...
tui.select.set()  { local id="$1"; shift; _tui_wx.arr "$id"; _WXA=("$@"); _WXSEL[$id]=-1
                    local i; for i in "${!_WXA[@]}"; do [[ "${_WXA[i]}" == "${_TUI_W_VALUE[$id]}" ]] && _WXSEL[$id]=$i; done
                    _tui_wx.redraw "$id"; }
tui.select.index() { printf '%s\n' "${_WXSEL[$1]:--1}"; }
tui.select.pick()  {
    _tui_wx.arr "$1"; local i=$2
    (( i >= 0 && i < ${#_WXA[@]} )) || return 1
    _WXSEL[$1]=$i; _TUI_W_VALUE[$1]="${_WXA[i]}"; _tui_wx.redraw "$1"; _tui_wx.changed "$1"
    local fn="${_TUI_W_ACTION[$1]:-}"; [[ -n "$fn" ]] && "$fn" "$1"
    return 0
}
tui.progress.set() {
    local v=$2 m=${3:-100}
    (( m > 0 )) || m=100
    (( v = v * 100 / m )); (( v < 0 )) && v=0; (( v > 100 )) && v=100
    _TUI_W_VALUE[$1]=$v; _tui_wx.redraw "$1"
}

# select: open the picker (a tui.choose dialog); picking sets the value
_tui_wx.select_open() {
    local id="$1"; _tui_wx.arr "$id"
    (( ${#_WXA[@]} )) || return 0
    _WX_TARGET="$id"
...
# ── geometry helpers ─────────────────────────────────────────────────────
_tui_wx.multirow() { case "${_TUI_W_TYPE[$1]:-}" in textarea|list|table) return 0 ;; esac; return 1; }
...
_tui_wx.move() {
    ...
    _tui._widget_pos "$id"; h=$_WSH; _WXH[$id]=$h
    _tui_wx.arr "$id"; n=${#_WXA[@]}
    (( n )) || return 1
    old=$sel
    ...
    case "$k" in
        ...
        down) (( sel < n - 1 )) && (( sel++ )) ;;
        pgup) (( sel -= h - 1 )); (( sel < 0 )) && sel=0 ;;
        pgdn) (( sel += h - 1 )); (( sel >= n )) && sel=$(( n - 1 )) ;;
        home) sel=0 ;;
        end)  sel=$(( n - 1 )) ;;
    esac
    (( sel == old )) && return 1
    _WXSEL[$id]=$sel; _WXF[$id]=1
    _tui_wx.changed "$id"
    return 0
}
...
_tui_wx.consumes() {
    ...
    case "$type" in
        input|password|textarea) _tui_text.consumes "$id" "$name"; return ;;
        list|table)
            _tui_wx.arr "$id"; n=${#_WXA[@]}; sel=${_WXSEL[$id]:--1}
            case "$name" in
                up)        (( n && sel > 0 )); return ;;
                down)      (( n && sel < n - 1 )); return ;;
                pgup|pgdn|home|end) (( n )); return ;;
            esac ;;
...
_tui_wx.key() {
    ...
    case "$type" in
        ...
        list|table) _tui_wx.move "$id" "$name"; _tui_wx.redraw "$id" ;;
...
_tui_wx.mouse() {   # ID X Y KIND(press|drag)
    ...
    case "$type" in
        ...
        list|table)
            ...
            _tui_wx.arr "$id"
            (( i >= 0 && i < ${#_WXA[@]} )) || return 0
            now=${EPOCHREALTIME//[.,]/}; now=$(( now / 1000 ))
            local same=0
            [[ "$_WX_CLICK_ID" == "$id" && "$_WX_CLICK_I" == "$i" ]] && (( now - _WX_CLICK_T < 450 )) && same=1
            _WX_CLICK_T=$now; _WX_CLICK_ID="$id"; _WX_CLICK_I=$i
            if (( _WXSEL[$id] != i )); then _WXSEL[$id]=$i; _tui_wx.changed "$id"; fi
            _tui_wx.redraw "$id"
            if (( same )); then local fn="${_TUI_W_ACTION[$id]:-}"; [[ -n "$fn" ]] && "$fn" "$id"; _WX_CLICK_T=0; fi
            return 0 ;;
        select) _tui_wx.select_open "$id"; return 0 ;;
    esac
    return 1
}
...
_tui_wx.wheel() {   # DIRECTION N
    ...
    _tui._widget_pos "$id"; _WXH[$id]=$_WSH; _TXH[$id]=$_WSH
    local total top h old area=0
    [[ "${_TUI_W_TYPE[$id]}" == textarea ]] && area=1
    if (( area )); then _tui_text.line_count "$id"; total=$_LC; top=${_TXT[$id]:-0}; h=$_WSH
    else _tui_wx.arr "$id"; total=${#_WXA[@]}; top=${_WXTOP[$id]:-0}; h=$_WSH; [[ "${_TUI_W_TYPE[$id]}" == table ]] && (( h-- )); fi
    old=$top
    case "$1" in
        up)    (( top -= n * c )) ;;
        down)  (( top += n * c )) ;;
        left|right)
            (( area )) || return 1
            local l=${_TXS[$id]:-0}; old=$l
            [[ "$1" == left ]] && (( l -= n * c )) || (( l += n * c ))
            (( l < 0 )) && l=0
            (( l == old )) && return 1
            _TXS[$id]=$l; _TXF[$id]=0; _tui_wx.redraw "$id"; return 0 ;;
        *)     return 1 ;;
    esac
    (( top > total - h )) && top=$(( total - h )); (( top < 0 )) && top=0
    (( top == old )) && return 1
    if (( area )); then _TXT[$id]=$top; _TXF[$id]=0; else _WXTOP[$id]=$top; _WXF[$id]=0; fi
...
_tui_wx.draw_select() {
    ...
    [[ -z "$v" ]] && v="(none)"
    [[ -n "$label" ]] && { lw=$(( ${#label} + 1 )); style.bold; printf '%s ' "$label"; style.reset; }
    _tui._apply_style "$sk" "$pk"
    (( focused )) && style.reverse || { [[ -z "${_TUI_STYLE_FG[$sk]:-}" ]] && style.dim; }
    local fw=$(( sw - lw ))
    line="[ ${v} ▾ ]"
    _tui_text.padc "$line" "$fw"; printf '%s' "$_PADC"
    style.reset
}
...
_tui_wx.draw_progress() {
    ...
    [[ "$pct" =~ ^[0-9]+$ ]] || pct=0
    (( pct > 100 )) && pct=100
    [[ -n "$label" ]] && { lw=$(( ${#label} + 1 )); printf '%s ' "$label"; }
    bw=$(( sw - lw - 5 )); (( bw < 3 )) && bw=3
    fill=$(( bw * pct / 100 ))
    tui.class.sgr progress_fill; local f="${TUI_SGR:-$'\e[0;32m'}"
    tui.class.sgr progress;      local e="${TUI_SGR:-$'\e[2;37m'}"
    local fs es
    printf -v fs '%*s' "$fill" ''; printf -v es '%*s' "$(( bw - fill ))" ''
...
_tui_wx.draw_rows() {
    ...
    if (( hdr )); then
        ...
        total=0; for c in "${!cw[@]}"; do (( total += cw[c] + 2 )); done
        while (( total > fw )); do
            maxc=0; for c in "${!cw[@]}"; do (( cw[c] > cw[maxc] )) && maxc=$c; done
            (( cw[maxc] > 3 )) || break
            (( cw[maxc]-- )); (( total-- ))
        done
    fi

    _tui._apply_style "$sk" "$pk"; _tui._style_v "$sk" "$pk"; local base="$_SGR"
    local cls=list_sel; (( hdr )) && cls=table_sel
    tui.class.sgr "$cls"; local selsgr="${TUI_SGR:-$'\e[0;97;48;2;62;92;138m'}"
    local rowi=0
    if (( hdr )); then
        cur.goto "$sr" "$sc"
        tui.class.sgr table_head; local hs="${TUI_SGR:-$'\e[1;4m'}"
        IFS='|' read -ra cells <<< "${_WXCOLS[$id]}"
        line=""; for c in "${!cw[@]}"; do _tui_text.padc "${cells[c]}" "${cw[c]}"; line+="$_PADC  "; done
        _tui_text.padc "$line" "$fw"; line="$_PADC"
    ...
    for (( i = 0; i < rows; i++ )); do
        ...
        if (( idx < n )); then
            ...
            _tui_text.padc "$line" "$fw"; line="$_PADC"
            if (( idx == sel )); then
                if (( focused )); then printf '%s%s%s%s' "$selsgr" "$line" $'\e[0m' "$base"
...
_tui_wx.reset() {
    _TUI_W_ROWSPAN=(); _WXSEL=(); _WXTOP=(); _WXCOLS=(); _WXH=(); _TUI_W_CHANGE=()
    _TXC=(); _TXA=(); _TXS=(); _TXT=(); _TXW=(); _TXH=(); _TXUV=(); _TXUC=(); _TXUN=(); _TXRV=(); _TXRC=(); _TXRN=(); _TXLK=(); _TXLP=()
    _TXG_R=(); _TXG_C=(); _TXG_W=(); _TXG_H=(); _TX_DRAG=""
}
_tui_wx.forget() {
    local id="$1"
...
_markup_wx() {
    ...
    id="$(_markup_attr "$line" id)"; pane="$(_markup_attr "$line" pane)"; row="$(_markup_attr "$line" row)"
    rows="$(_markup_attr "$line" rows)"
    local -a arr=()
    case "$tag" in
        password) tui.password "$id" "$pane" "$row" "$(_markup_attr "$line" placeholder)" "$(_markup_attr "$line" label)" "$(_markup_attr "$line" submit)"
                  tui.label_align "$id" "$(_markup_attr "$line" label_align)"; tui.label_width "$id" "$(_markup_attr "$line" label_width)" ;;
        textarea) tui.textarea "$id" "$pane" "$row" "$(_markup_attr "$line" placeholder)" "${rows:-0}" "$(_markup_attr "$line" submit)"
                  local v; v="$(_markup_attr "$line" value)"; [[ -n "$v" ]] && tui.set "$id" "${v//\\n/$'\n'}" ;;
        list)     tui.list "$id" "$pane" "$row" "$(_markup_attr "$line" action)" "${rows:-0}"
                  items="$(_markup_attr "$line" items)"
                  if [[ -n "$items" ]]; then IFS='|' read -ra arr <<< "$items"; tui.list.set "$id" "${arr[@]}"; fi ;;
        table)    tui.table "$id" "$pane" "$row" "$(_markup_attr "$line" action)" "${rows:-0}"
                  data="$(_markup_attr "$line" data)"
                  if [[ -n "$data" ]]; then IFS=';' read -ra arr <<< "$data"; fi
                  tui.table.set "$id" "$(_markup_attr "$line" columns)" "${arr[@]}" ;;
        select)   tui.select "$id" "$pane" "$row" "$(_markup_attr "$line" label)" "$(_markup_attr "$line" action)"
                  local sv; sv="$(_markup_attr "$line" value)"; [[ -n "$sv" ]] && tui.set "$id" "$sv"
                  items="$(_markup_attr "$line" items)"
                  if [[ -n "$items" ]]; then IFS='|' read -ra arr <<< "$items"; tui.select.set "$id" "${arr[@]}"; fi ;;
        progress) tui.progress "$id" "$pane" "$row" "$(_markup_attr "$line" label)"
                  local pv; pv="$(_markup_attr "$line" value)"; [[ -n "$pv" ]] && tui.set "$id" "$pv" ;;
    esac
    local oc; oc="$(_markup_attr "$line" on_change)"; [[ -n "$oc" ]] && tui.on_change "$id" "$oc"
