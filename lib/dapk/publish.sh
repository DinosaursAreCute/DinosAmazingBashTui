...
_dapk.publish.api() {
	...
	code="$(printf 'header = "Authorization: Bearer %s"\n' "$tok" | curl "${args[@]}" "$url" 2>>"${DAPK_UI_LOG_FILE:-/dev/null}")" || {
		rm -f "$tmp"
		DAPK_PUB_ERROR="network error calling $url"
		return 1
	}
	_PUB_BODY="$(<"$tmp")"
	rm -f "$tmp"
	...
	[[ "$code" =~ ^2 ]] && return 0
	local msg=""
	[[ "$_PUB_BODY" =~ \"message\":\"([^\"]*)\" ]] && msg="${BASH_REMATCH[1]}"
	DAPK_PUB_ERROR="GitHub API $method ${url#*://*/} -> HTTP $code${msg:+: $msg}"
	return 1
}
...
dapk.publish.run() {
	...
	[[ "$repo" =~ ^[A-Za-z0-9._-]+/[A-Za-z0-9._-]+$ ]] || {
		DAPK_PUB_ERROR="no GitHub repository: set publish_repo in dabt.pkg (owner/name) or a github.com origin remote"
		return 1
	}
	if ((DAPK_PUB_PRE)); then
		[[ "$mode" == release ]] && {
			DAPK_PUB_ERROR="$tag is a pre-release version: a full release is refused (use --prerelease or --draft)"
			return 3
		}
	...
	case "$mode" in
		...
		*)
			DAPK_PUB_ERROR="unknown publish mode '$mode'"
			return 2
			;;
	esac
	body=""
	[[ -r "$DAPK_PUB_BODY" ]] && body="$(<"$DAPK_PUB_BODY")"
	dapk.ui.kv repo "$repo"
	dapk.ui.kv tag "$tag"
	dapk.ui.kv mode "$mode (draft=$draft prerelease=$prerelease)"
	for a in "${DAPK_PUB_ASSETS[@]}"; do dapk.ui.kv asset "${a##*/}"; done
	((DAPK_PUB_DRY)) && {
		dapk.ui.info "dry run: nothing was sent"
		return 0
	}
	[[ -n "${GITHUB_TOKEN:-${GH_TOKEN:-}}" ]] || {
		DAPK_PUB_ERROR="GITHUB_TOKEN (or GH_TOKEN) is not set"
		return 1
	}
	command -v curl >/dev/null 2>&1 || {
		DAPK_PUB_ERROR="curl is required to publish"
		return 1
	}

	# the release JSON goes through a file so nothing sensitive or large sits in argv
	local jf
	jf="$(mktemp "${TMPDIR:-/tmp}/dapk_rel.XXXXXX")" || return 1
	...
	for page in 1 2 3 4 5; do
		_dapk.publish.api GET "$api/repos/$repo/releases?per_page=100&page=$page" || {
			rm -f "$jf"
			return 1
		}
		json="$_PUB_BODY"
		[[ "$json" == "[]" || -z "$json" ]] && break
		id="$(_dapk.publish.find_release "$json" "$tag")"
		[[ -n "$id" ]] && break
	done
	if [[ -n "$id" ]]; then
		dapk.ui.info "updating the existing release for $tag"
		_dapk.publish.api PATCH "$api/repos/$repo/releases/$id" --data "$jf" || {
			rm -f "$jf"
			return 1
		}
	else
		_dapk.publish.api POST "$api/repos/$repo/releases" --data "$jf" || {
			rm -f "$jf"
			return 1
		}
	...
	[[ -n "$id" ]] || {
		DAPK_PUB_ERROR="could not read the release id from the GitHub response"
		return 1
	}
