...
tui.install.check_dir() { # DIR -> rc 0 ok; TUI_INSTALL_ERROR says why not
	...
	TUI_INSTALL_ERROR=""
	[[ -n "$d" ]] || {
		TUI_INSTALL_ERROR="empty path"
		return 1
	}
	[[ "$d" == /* ]] || {
		TUI_INSTALL_ERROR="use an absolute path (starting with /)"
		return 1
	}
	if [[ -e "$d" && ! -d "$d" ]]; then
		TUI_INSTALL_ERROR="$d is a file"
		return 1
	fi
	if [[ -d "$d" ]]; then
		[[ -w "$d" ]] || {
			TUI_INSTALL_ERROR="$d is not writable"
			return 1
		}
		return 0
	fi
	...
	[[ -w "$parent" ]] || {
		TUI_INSTALL_ERROR="cannot create $d ($parent is not writable)"
		return 1
	}
	return 0
}
...
tui.install.run() {
	...
	TUI_INSTALL_LOG=()
	TUI_INSTALL_ERROR=""
	TUI_SYNC_POLICY="$policy"
	src="$(cd -P "$src" 2>/dev/null && pwd -P)" || {
		TUI_INSTALL_ERROR="source folder not found"
		return 2
	}
	tui.sync.valid_source "$src" || {
		TUI_INSTALL_ERROR="$src is not a DABT release (needs VERSION, lib/tui.sh, share/defaults)"
		return 2
	}
	...
	local same_tree=0
	[[ -d "$prefix" && "$prefix" -ef "$src" ]] && same_tree=1 # a checkout used in place: only the config is installed
	...
	local stamp
	if [[ -n "${TUI_SYNC_STAMP:-}" ]]; then stamp="$TUI_SYNC_STAMP"; else printf -v stamp '%(%Y%m%d-%H%M%S)T' -1; fi
	TUI_SYNC_STAMP="$stamp"
	mkdir -p "$conf" "$prefix" || {
		TUI_INSTALL_ERROR="cannot create the folders"
		return 1
	}
	tui.sync.apply "$src" "$conf" "$resolver" || {
		TUI_INSTALL_ERROR="config sync failed"
		return 1
	}
	if ((same_tree)); then
		_tui_install.log "program: left in place ($prefix is the source folder)"
	else
		tui.sync.program_apply "$src" "$prefix" "$conf/backups/$stamp" || {
			TUI_INSTALL_ERROR="program copy failed"
			return 1
		}
	...
	if ((link)) && [[ -n "$bindir" ]]; then
		if mkdir -p "$bindir" 2>/dev/null && ln -sfn "$prefix/bin/dabt" "$bindir/dabt" 2>/dev/null; then
			_tui_install.log "launcher: $bindir/dabt"
