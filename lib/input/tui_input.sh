...
_tui_input.name_char() {
    ...
    case "$c" in
        ...
        $'\n'|$'\r')    _KEY=enter ;;
        $'\x7f')       _KEY=backspace ;;
        $'\b')         _KEY=ctrl+backspace ;;      # most terminals send ^H for ctrl+backspace
        ' ')            _KEY=space ;;
        *)
            printf -v code '%d' "'$c"
...
_tui_input.name_seq() {
    ...
    case "$seq" in
        ...
        "[a") _KEY="shift+up"; return ;;  "[b") _KEY="shift+down"; return ;;      # rxvt: ESC [ a-d = shift+arrow
        "[c") _KEY="shift+right"; return ;; "[d") _KEY="shift+left"; return ;;
        "["*)
            ...
            return ;;
        "O"?)   # SS3 (application mode): arrows, F1-F4; rxvt: lowercase = ctrl+arrow
            case "${seq:1:1}" in
                a) _KEY=ctrl+up ;; b) _KEY=ctrl+down ;; c) _KEY=ctrl+right ;; d) _KEY=ctrl+left ;;
                A) _KEY=up ;; B) _KEY=down ;; C) _KEY=right ;; D) _KEY=left ;;
                H) _KEY=home ;; F) _KEY=end ;;
                P) _KEY=f1 ;; Q) _KEY=f2 ;; R) _KEY=f3 ;; S) _KEY=f4 ;;
            esac
...
tui.bind() {
    ...
    [[ -n "$user" ]] || _tui_plugin.own bind "$spec${pane:+ --pane $pane}"
    if [[ -n "$user" ]]; then
        _TUI_UBIND[$id]="$cmd"
...
_tui_input.key_event() {
    ...
    if (( _TUI_KEYS_SUSPENDED )); then
        ...
        if [[ "$name" == enter && -n "$_TUI_FOCUS_ID" ]] && _tui_text.is_text "$_TUI_FOCUS_ID" && ! _tui_text.multiline "$_TUI_FOCUS_ID"; then
            tui.action.activate; return 0
        fi
        _tui_input.feed_text "$name" && return 0
        return 1
    fi
    local rc=0
    _tui_input.dispatch "$name" || rc=$?
    (( rc )) && _tui_input.feed_text "$name" && return 0
    return $rc
}

# an unbound key for a focused text widget / list: let the widget have it. rc 0 when it did.
_tui_input.feed_text() {
    [[ -n "$_TUI_FOCUS_ID" ]] || return 1
    _tui_wx.consumes "$_TUI_FOCUS_ID" "$1" || return 1
    _tui_wx.key "$_TUI_FOCUS_ID" "$1"
}
...
# tui.action.quit asks first when the "confirm.quit" setting is on (tui.config.set confirm.quit 1; the default Settings
# page has a checkbox for it). tui.action.quit_now never asks.
tui.action.quit()       {
    if (( _TUI_RUNNING )) && [[ "${_TUI_CFG[confirm.quit]:-0}" == 1 ]] && ! tui.dialog.active; then
        tui.confirm "Quit ${TUI_APP_NAME:-the application}?" tui.stop --title Quit --yes Quit --no Stay
    else tui.stop; fi
}
...
tui.action.activate() {
    ...
    case "$ftype" in
        ...
        list|table) [[ -n "$faction" ]] && "$faction" "$_TUI_FOCUS_ID" ;;
        ...
        input|password|textarea)
            ...
            if [[ "$_TUI_FOCUS_ID" == "$wid" && -n "${_TUI_W_TYPE[$wid]:-}" ]]; then
                if [[ "${_TUI_W_RETAIN[$wid]:-$TUI_INPUT_RETAIN_ON_SUBMIT}" != 1 ]]; then _tui._unfocus
                    ...
                    _tui_text.norm "$wid"                       # the callback may have cleared the text
...
# alt+arrows: scroll the pane when it can scroll that way (so it works while a text box has the keyboard), otherwise
# move keyboard focus to the neighbouring pane as before
tui.action.scroll_or_pane() {
    local dir="$1" mode
    if _tui_input.scroll_target; then
        mode="${_TUI_P_SCROLL[$_ST]:-none}"
        case "$dir" in
            left|right) [[ "$mode" == h || "$mode" == both ]] && { tui.action.scroll "$dir"; return 0; } ;;
            up|down)    [[ "$mode" == v || "$mode" == both ]] && { tui.action.scroll "$dir"; return 0; } ;;
        esac
    fi
    tui.action.pane_dir "$dir"
}
...
tui.action.click() {
    ...
    if [[ "$TUI_EVENT_KEY" == drag:* ]]; then          # a drag selects text in the focused text widget; otherwise it only moves scrollbars
        [[ -n "$_TUI_FOCUS_ID" ]] && _tui_text.is_text "$_TUI_FOCUS_ID" && _tui_text.drag "$_TUI_FOCUS_ID" "$mx" "$my"
        return
    fi
    ...
    if [[ -n "$hover_widget" ]]; then
        ...
        case "$wtype" in
            ...
            input|password|textarea|list|table|select)
                tui.focus "$wid"
...
# ── defaults (loaded from share/defaults/keybinds.xml) ──────────────────
# The built-in bindings are DATA, not code: one <bind key= action= group= [always=] [desc=]/> per line
# in ${TUI_DEFAULTS_DIR}/keybinds.xml (default: <repo>/share/defaults). Each belongs to a GROUP, and a
...
declare -g  TUI_DEFAULTS_DIR="${TUI_DEFAULTS_DIR:-$TUI_ROOT/share/defaults}"        # normally set by tui_home.sh
...
# tui.action.paste - insert the pasted text into the focused text widget (replaces the selection; single-line widgets
# turn newlines into spaces and drop control characters, a textarea keeps its newlines).
tui.action.paste() {
    [[ -n "$_TUI_FOCUS_ID" ]] && _tui_text.is_text "$_TUI_FOCUS_ID" || return 0
    tui.text.insert "$_TUI_FOCUS_ID" "$TUI_EVENT_PASTE" paste
    _tui._draw_widget "$_TUI_FOCUS_ID"
}
...
# File: $TUI_APP_CONF/keybinds.xml = ~/.config/DABT/apps/<app>/keybinds.xml - the same <bind .../> line format as
# share/defaults/keybinds.xml. Set TUI_APP_NAME (before sourcing tui.sh) to give your app its own directory.
declare -g  TUI_APP_NAME="${TUI_APP_NAME:-dabt}"
declare -g  TUI_USER_KEYBINDS="${TUI_USER_KEYBINDS:-$TUI_APP_CONF/keybinds.xml}"
