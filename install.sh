...
if [[ ! -f "$SRC/lib/apps/tui_sync.sh" ]]; then
	...
	repo="${DABT_REPO:-DinosaursAreCute/DinosAmazingBashTui}"
	ref="${DABT_REF:-main}"
	url="https://github.com/$repo/archive/refs/heads/$ref.tar.gz"
	[[ "$ref" == v[0-9]* ]] && url="https://github.com/$repo/archive/refs/tags/$ref.tar.gz"
	...
	if command -v curl >/dev/null 2>&1; then
		curl -fsSL -o "$tmp/dabt.tar.gz" "$url"
	elif command -v wget >/dev/null 2>&1; then
		wget -q -O "$tmp/dabt.tar.gz" "$url"
	else
	...
	mkdir "$tmp/src" && tar -xzf "$tmp/dabt.tar.gz" -C "$tmp/src" --strip-components=1 || {
		echo "install.sh: bad archive" >&2
		exit 2
	}
...
_scan() {
	...
	if ((TUI_SCAN_HIGH)); then
		...
		((force)) && _warn "--force given: installing anyway" || {
			echo "install.sh: refusing to install; review the findings, then rerun with --force" >&2
			exit 3
		}
	elif ((TUI_SCAN_WARN)); then
		_warn "no high-risk findings, $TUI_SCAN_WARN warning(s)"
	else _ok "no findings"; fi
}
_run() {
	local args=(--bindir "$TUI_INSTALL_BINDIR")
	((link)) || args=(--no-link)
	((dry)) && args+=(--dry-run)
...
_finish() {
	local rc=$1 l
	if ((rc != 0)); then
		_err "$TUI_INSTALL_ERROR"
		exit "$rc"
	fi
	if ((dry)); then
		_ok "Would install DABT $(<"$SRC/VERSION")"
		_kv program "$TUI_INSTALL_PREFIX"
		_kv config "$TUI_INSTALL_CONFIG"
		tui.sync.report
...
_kv program "$TUI_INSTALL_PREFIX"
_kv config "$TUI_INSTALL_CONFIG"
_kv command "$TUI_INSTALL_BINDIR/dabt"
...
if ((! yes && ! dry)); then
	if ((interactive)); then
		...
		echo "install.sh: not a terminal and no --yes: refusing to guess. Use --yes." >&2
		exit 2
	fi
fi
((noscan)) || _scan
