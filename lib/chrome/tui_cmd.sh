...
# default actions are registered through exactly the same calls an app uses (share/defaults/commands.xml
...
declare -gA _TUI_CMD_PLUGIN=()            # command id -> the plugin that registered it
...
tui.cmd.add() {
    ...
    (( _TUI_CMD_IN_PROVIDER )) || _tui_plugin.own cmd "$id"
    if [[ -n "${_TPL_CUR:-}" ]]; then _TUI_CMD_PLUGIN[$id]="$_TPL_CUR"; else unset '_TUI_CMD_PLUGIN[$id]'; fi
...
tui.cmd.remove() {
    ...
    _TUI_CMD_IDS=("${keep[@]}")
    unset '_TUI_CMD_PLUGIN[$1]' '_TUI_CMD_TITLE[$1]' '_TUI_CMD_ACTION[$1]' '_TUI_CMD_GROUP[$1]' '_TUI_CMD_DESC[$1]' '_TUI_CMD_WHEN[$1]' '_TUI_CMD_DYN[$1]'
}
...
# tui.cmd.load FILE - one <cmd .../> per line (same style as share/defaults/keybinds.xml)
tui.cmd.load() {
...
_tui_cmd.refresh() {
    ...
    for id in "${_TUI_CMD_IDS[@]}"; do
        ...
        [[ -z "${_PAL_HINT[$id]}" && -n "${_TUI_CMD_PLUGIN[$id]:-}" ]] && _PAL_HINT[$id]="plugin: ${_TUI_CMD_PLUGIN[$id]}"      # show where a plugin's command comes from
    done
}
...
tui.cmd.load "$TUI_DEFAULTS_DIR/commands.xml" 2>/dev/null

# ── pages that ship with DABT (share/defaults/pages/) ────────────────────
