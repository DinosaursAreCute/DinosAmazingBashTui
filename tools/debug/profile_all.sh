...
#   tools/debug/profile_all.sh [-d PAGES_DIR] [-r ROWSxCOLS] [-n ITER] [-s SECTIONS] [PAGE...]
...
pid() {
	local a
	read -r a a a a _pid </proc/loadavg
} # sets _pid = last pid created
rss() {
	local l
	while read -r l; do [[ "$l" == VmRSS:* ]] && {
		_RSS="${l//[^0-9]/}"
		return
...
if (($#)); then pages=("$@"); else
	...
	for f in "$PAGES_DIR"/*.xml; do
		b="${f##*/}"
		[[ "$b" == _* || "$b" == commands.xml ]] || pages+=("${b%.xml}")
	done
fi

want startup && {
	hdr startup
	printf '  source tui.sh: %d ms   RSS after source: %d kB   bash %s\n' $(((S1 - S0) / 1000)) "$_RSS" "$BASH_VERSION"
}
...
if want pages; then
	...
	for p in "${pages[@]}"; do
		...
		if declare -p _TUI_CACHE_PAGE >/dev/null 2>&1; then
			_TUI_CACHE_PAGE=()
			_TUI_CACHE_SIG=()
		fi
		us a
		...
		us b
		cold=$(((b - a) / 1000)) # cold: parse + record + on_visit
		tui.goto "$PAGES_DIR/$home.xml" >/dev/null 2>&1
		pid
		p0=$_pid
		us t0
		...
		key="${f}"
		cb=0
		for k in "${!_TUI_CACHE_PAGE[@]}"; do [[ "$k" == *"/$p.xml" ]] && cb=${#_TUI_CACHE_PAGE[$k]}; done
		fb=$(wc -c <"$SINK")
...
if want calls; then
	...
	tui.goto "$PAGES_DIR/$home.xml" >/dev/null 2>&1
	first_pane="${_TUI_P_LEAVES[0]:-root}"
	first_w="${_TUI_W_ORDER[0]:-}"
...
if want cache; then
	hdr cache
	f="$PAGES_DIR/components.xml"
	[[ -f "$f" ]] || f="$PAGES_DIR/${pages[0]}.xml"
	tui.goto "$f" >/dev/null 2>&1
	tui.goto "$PAGES_DIR/$home.xml" >/dev/null 2>&1
	...
	printf '  cached pages: %d   in-memory bytes: ' "${#_TUI_CACHE_PAGE[@]}"
	tot=0
	for k in "${!_TUI_CACHE_PAGE[@]}"; do ((tot += ${#_TUI_CACHE_PAGE[$k]})); done
	echo "$tot"
	...
	for v in $(compgen -A variable _TUI_ | sort); do
		...
		case "$decl" in
			"declare -a"* | "declare -A"* | "declare -ga"* | "declare -gA"*)
				local_n=$(eval "echo \${#$v[@]}")
				((local_n > 20)) && printf '  %-34s %6d entries\n' "$v" "$local_n"
				;;
		esac
	done
...
if want resize; then
	...
	for sz in 24x80 45x150 60x200 24x120 45x150; do
		...
		_tui._root_h
		us a
		_tui._layout root >/dev/null 2>&1
		us b
		tui.render >"$SINK" 2>&1
		us c
