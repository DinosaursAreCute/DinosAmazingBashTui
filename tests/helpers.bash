...
MOCK
	...
	printf '#!/bin/sh\nexit 1\n' >"$MOCKBIN/wget"
	chmod +x "$MOCKBIN/wget" # never used, but must not reach the real one
}
...
dapk_setup() {
	...
	FP="$(ssh-keygen -lf "$T/key" | awk '{print $2}')"
	export FP
}
...
make_project() {
	...
	printf '#!/usr/bin/env bash\necho hi\n' >"$d/app.sh"
	chmod +x "$d/app.sh"
	echo x >"$d/config/a.xml"
	echo lic >"$d/LICENSE"
	echo one >"$d/assets/one.txt"
	printf '# Changelog\n\n## [Unreleased]\n### News\n- Shiny new thing\n- Two line\n  bullet\n### Fixed\n- a bug\n\n## [1.2.2] - 2026-01-01\n### News\n- Old thing\n' >"$d/CHANGELOG.md"
...
build_project() {
	local d="$1"
	shift
	(cd "$d" && DABT_SIGN_KEY="$(<"$T/key")" bash "$DABT" build "$@")
}
