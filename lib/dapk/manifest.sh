...
_dapk.manifest.tool() {
	...
	if command -v sha256sum >/dev/null 2>&1; then
		...
		_DM_SHA=(shasum -a 256)
	else
		DAPK_MANIFEST_ERROR="sha256sum or shasum is required"
		return 1
	fi
}
...
_dapk.manifest.dhash() { # DIR -> _DH
	...
	while IFS= read -r kid; do
		...
		path="${d:+$d/}$n"
		if [[ "$t" == d ]]; then
			_dapk.manifest.dhash "$path"
			h="$_DH"
			m="-"
		else
			h="${_DM_HASH[$path]}"
			if [[ -x "$_DM_TREE/$path" ]]; then m=755; else m=644; fi
		fi
		out+="$t $h $m $n"$'\n'
...
dapk.manifest.scan() {
	...
	while IFS= read -r -d '' p; do
		p="${p#./}"
		[[ "$p" == MANIFEST || "$p" == MANIFEST.sig ]] && continue
		[[ "$p" == *$'\n'* || "$p" == *\\* ]] && {
			DAPK_MANIFEST_ERROR="unsupported character in file name: $p"
			return 1
		}
		files+=("$p")
	...
	if ((total)); then
		while IFS= read -r line; do
			...
			p="${p#./}"
			[[ "$line" == \\* ]] && {
				DAPK_MANIFEST_ERROR="unsupported file name in tree"
				return 1
			}
			_DM_HASH[$p]="$h"
			((DAPK_MANIFEST_PROGRESS)) && dapk.ui.progress_tick
		done < <(cd "$tree" && printf '%s\0' "${files[@]/#/./}" | xargs -0 "${_DM_SHA[@]}" 2>/dev/null)
		((${#_DM_HASH[@]} == total)) || {
			DAPK_MANIFEST_ERROR="hashing failed (${#_DM_HASH[@]} of $total files)"
			return 1
		}
	fi
	...
	while IFS= read -r -d '' p; do
		p="${p#./}"
		[[ "$p" == . || -z "$p" ]] && continue
		_dapk.manifest.addpath "$p" d
	done < <(cd "$tree" && find . -mindepth 1 -type d -print0)
	while IFS= read -r kid; do
		[[ -n "$kid" ]] || continue
		t="${kid%%$'\t'*}"
		n="${kid#*$'\t'}"
		if [[ "$t" == d ]]; then
			_dapk.manifest.dhash "$n"
			DAPK_MANIFEST_ENTRIES+=("d $_DH - - $n")
		else
			size="$(wc -c <"$tree/$n")"
			size="${size//[[:space:]]/}"
...
dapk.manifest.read() {
	...
	DAPK_MANIFEST_ERROR=""
	[[ -r "$f" ]] || {
		DAPK_MANIFEST_ERROR="cannot read $f"
		return 1
	}
	while IFS= read -r line || [[ -n "$line" ]]; do
		if ((first)); then
			first=0
			[[ "$line" == "$DAPK_MANIFEST_MAGIC" ]] || {
				DAPK_MANIFEST_ERROR="not a DABT manifest (missing '$DAPK_MANIFEST_MAGIC')"
				return 1
			}
			continue
		fi
		if ((inhdr)); then
			...
			[[ "$line" == *=* ]] || {
				DAPK_MANIFEST_ERROR="bad header line: $line"
				return 1
			}
			DAPK_MANIFEST_H[${line%%=*}]="${line#*=}"
			continue
		fi
		[[ -n "$line" ]] || continue
		read -r t h s m p <<<"$line"
		[[ ("$t" == f || "$t" == d) && "$h" =~ ^[0-9a-f]{64}$ && -n "$p" ]] || {
			DAPK_MANIFEST_ERROR="bad entry line: $line"
			return 1
		}
		DAPK_MANIFEST_ENTRIES+=("$line")
	done <"$f"
	((inhdr)) && {
		DAPK_MANIFEST_ERROR="manifest has no '---' separator"
		return 1
	}
	[[ -n "${DAPK_MANIFEST_H[name]:-}" && -n "${DAPK_MANIFEST_H[version]:-}" ]] || {
		DAPK_MANIFEST_ERROR="manifest lacks name or version"
		return 1
	}
	return 0
}
...
dapk.manifest.compare() {
	...
	for e in "${DAPK_MANIFEST_COMPUTED[@]}"; do
		read -r t h s m p <<<"$e"
		have[$p]="$t $h $s $m"
	done
	for p in "${!want[@]}"; do
		[[ -n "${have[$p]:-}" ]] || {
			DAPK_MANIFEST_PROBLEMS+=("missing from the package: $p")
			continue
		}
