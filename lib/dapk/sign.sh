...
dapk.sign.generate() {
	...
	command -v ssh-keygen >/dev/null 2>&1 || {
		DAPK_SIGN_ERROR="ssh-keygen (OpenSSH) is required"
		return 1
	}
	[[ ! -e "$k" ]] || {
		DAPK_SIGN_ERROR="$k already exists"
		return 1
	}
	mkdir -p "$(dirname "$k")" && chmod 700 "$(dirname "$k")" 2>/dev/null
	ssh-keygen -q -t ed25519 -N "" -C "dabt-pkg" -f "$k" </dev/null >/dev/null 2>&1 && chmod 600 "$k" || {
		DAPK_SIGN_ERROR="key generation failed"
		return 1
	}
	DAPK_SIGN_KEYFILE="$k"
}
...
dapk.sign.resolve_key() {
	DAPK_SIGN_ERROR=""
	command -v ssh-keygen >/dev/null 2>&1 || {
		DAPK_SIGN_ERROR="ssh-keygen (OpenSSH) is required for signing"
		return 1
	}
	if [[ -n "${1:-}" ]]; then
		[[ -r "$1" ]] || {
			DAPK_SIGN_ERROR="signing key not readable: $1"
			return 1
		}
		DAPK_SIGN_KEYFILE="$1"
		return 0
	...
	if [[ -z "${DABT_SIGN_KEY:-}" && -n "${2:-}" ]]; then
		[[ -r "$2" ]] || {
			DAPK_SIGN_ERROR="signing key not readable: $2 (sign_key in dabt.pkg)"
			return 1
		}
		DAPK_SIGN_KEYFILE="$2"
		return 0
	...
	if [[ -n "${DABT_SIGN_KEY:-}" ]]; then
		...
		chmod 700 "$_DAPK_SIGN_TMP"
		(
			umask 077
			printf '%s\n' "$DABT_SIGN_KEY" >"$_DAPK_SIGN_TMP/key"
		)
		chmod 600 "$_DAPK_SIGN_TMP/key"
		DAPK_SIGN_KEYFILE="$_DAPK_SIGN_TMP/key"
		return 0
	fi
	if [[ -r "$(dapk.sign.default_key)" ]]; then
		DAPK_SIGN_KEYFILE="$(dapk.sign.default_key)"
		return 0
	fi
	if [[ "${3:-0}" == 1 ]]; then
		dapk.sign.generate && return 0
		return 1
	fi
	DAPK_SIGN_ERROR="no signing key: set sign_key in dabt.pkg, pass --key FILE, export DABT_SIGN_KEY, or build with --auto-sign (or --no-sign)"
	return 1
}

dapk.sign.cleanup() {
	[[ -n "$_DAPK_SIGN_TMP" && -d "$_DAPK_SIGN_TMP" ]] && rm -rf "$_DAPK_SIGN_TMP"
	_DAPK_SIGN_TMP=""
}
...
dapk.sign.sign() {
	...
	rm -f "$manifest.sig"
	ssh-keygen -Y sign -q -f "$key" -n "$DAPK_SIG_NAMESPACE" "$manifest" >/dev/null 2>"${manifest}.signerr" || {
		DAPK_SIGN_ERROR="ssh-keygen failed: $(<"${manifest}.signerr")"
...
dapk.sign.inspect() {
	...
	command -v ssh-keygen >/dev/null 2>&1 || {
		DAPK_SIGN_ERROR="ssh-keygen (OpenSSH) is required to verify signatures"
		return 1
	}
	out="$(ssh-keygen -Y check-novalidate -n "$DAPK_SIG_NAMESPACE" -s "$sig" <"$manifest" 2>&1)" || {
		DAPK_SIGN_ERROR="the signature is invalid (${out##*$'\n'})"
		return 1
	}
	[[ "$out" =~ (SHA256:[A-Za-z0-9+/=]+) ]] && DAPK_SIGN_FPR="${BASH_REMATCH[1]}"
	[[ -n "$DAPK_SIGN_FPR" ]] || {
		DAPK_SIGN_ERROR="cannot read the signer from the signature"
		return 1
	}
	_dapk.sign.pubkey "$sig" || {
		DAPK_SIGN_ERROR="cannot read the public key from the signature"
		return 1
	}
	return 0
}
...
dapk.sign.trusted() {
	local f
	f="$(dapk.sign.trust_file)"
	[[ -r "$f" ]] && grep -q "^$1[[:space:]]" "$f"
}
