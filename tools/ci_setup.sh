...
set -u
key="${HOME}/.config/DABT/keys/dabt_ci_ed25519"; repo=""; gen=0
while (( $# )); do
    case "$1" in --key) key="$2"; shift ;; --repo) repo="$2"; shift ;; --generate) gen=1 ;;
        -h|--help) sed -n '2,10p' "$0" | sed 's/^# \?//'; exit 0 ;; *) echo "ci_setup: unknown option $1" >&2; exit 2 ;; esac
    shift
done
command -v gh >/dev/null 2>&1 || { echo "ci_setup: the GitHub CLI (gh) is required" >&2; exit 2; }
gh auth status >/dev/null 2>&1 || { echo "ci_setup: run  gh auth login  first" >&2; exit 2; }
[[ -n "$repo" ]] || repo="$(gh repo view --json nameWithOwner -q .nameWithOwner 2>/dev/null)"
[[ -n "$repo" ]] || { echo "ci_setup: cannot tell the repository; pass --repo OWNER/NAME" >&2; exit 2; }
if [[ ! -f "$key" ]]; then
    (( gen )) || { echo "ci_setup: $key does not exist (use --generate, or --key FILE)" >&2; exit 2; }
