...
theme="$1" size="$2" out="$3" pdir="$4"; shift 4
...
export TUI_APP_NAME=dabt_demo; mkdir -p "$XDG_CONFIG_HOME/DABT/apps/dabt_demo"; echo "theme=$theme" > "$XDG_CONFIG_HOME/DABT/apps/dabt_demo/settings.conf"   # settings page re-syncs its theme on visit
...
_TUI_RUNNING=1
_TUI_ROWS=$ROWS; _TUI_COLS=$COLS; _TUI_P_W[root]=$COLS; _tui._root_h; _TUI_P_ROW[root]=1; _TUI_P_COL[root]=1
...
_settle() {
    local n=$(( ${SETTLE%.*} * 10 + ${SETTLE#*.} )) i l; [[ "$SETTLE" == *.* ]] || n=$(( SETTLE * 10 ))
    (( n < 1 )) && n=1
    for (( i = 0; i < n; i++ )); do
        read -rt 0.1 <> <(:)
...
_shot() { _settle; tui.render >"$out/$1.ans" 2>/dev/null; }
# spec: PAGE | PAGE@TAB_ID | PAGE@*  (every tab of the page's tab group)
for spec in "$@"; do
    p="${spec%%@*}"; tab="${spec#*@}"; [[ "$spec" == *@* ]] || tab=""
    tui.goto "$pdir/$p.xml" >/dev/null 2>&1
    if [[ -z "$tab" ]]; then _shot "$p"; continue; fi
    tabs=("$tab")
    if [[ "$tab" == "*" ]]; then tabs=(); for t in $(printf '%s\n' "${!_TUI_TAB_GROUP[@]}" | sort -V); do tabs+=("$t"); done; fi
    for t in "${tabs[@]}"; do
        tui.tabs.activate "$t" >/dev/null 2>&1; _shot "${p}__${t}"
    done
done
