...
on_key_bind() {
	...
	if [[ "${a%% *}" != tui.action.* ]] && ! declare -F "${a%% *}" >/dev/null; then
		tui.update lbl_k_status "no such function: ${a%% *}"
		return
	fi
...
on_key_reset() {
	tui.bind.reset --user
	_keys_refresh
	tui.update lbl_k_status "all your binds removed (not saved yet)"
	_keys_dirty_label
}

on_key_save() {
	if tui.bind.save; then
		tui.update lbl_k_status "saved to ${TUI_USER_KEYBINDS/#$HOME/~}"
...
on_key_discard() {
	tui.bind.discard
	_keys_refresh
	tui.update lbl_k_status "unsaved changes discarded"
	_keys_dirty_label
}
