...
_dapk.pack.tar() {
    ...
    local t; for t in tar gtar; do command -v "$t" >/dev/null 2>&1 && "$t" --version 2>/dev/null | grep -q 'GNU tar' && { _DAPK_TAR="$t"; return 0; }; done
    DAPK_PACK_ERROR="GNU tar is required (reproducible archives); on macOS: brew install gnu-tar"; return 1
}
...
dapk.pack.create() {
    ...
    list="$tmp/list"
    ( cd "$parent" && { printf '%s\0' "$root/MANIFEST"; [[ -f "$root/MANIFEST.sig" ]] && printf '%s\0' "$root/MANIFEST.sig"
        find "$root" ! -path "$root/MANIFEST" ! -path "$root/MANIFEST.sig" -print0 | LC_ALL=C sort -z; } ) > "$list"
    n="$(tr -cd '\0' < "$list" | wc -c)"; n="${n//[[:space:]]/}"
    ...
    [[ -s "$tmp/pkg.tar" ]] || { DAPK_PACK_ERROR="tar failed: $(<"$tmp/err")"; rm -rf "$tmp"; return 1; }
    gzip -n -9 < "$tmp/pkg.tar" > "$out" || { DAPK_PACK_ERROR="gzip failed"; rm -rf "$tmp"; return 1; }
    rm -rf "$tmp"
...
dapk.pack.check() {
    ...
    local -a names types
    DAPK_PACK_ERROR=""; DAPK_PACK_ROOT=""; DAPK_PACK_COUNT=0
    [[ -f "$pkg" ]] || { DAPK_PACK_ERROR="not a file: $pkg"; return 1; }
    _dapk.pack.tar || return 1
    mapfile -t names < <("$_DAPK_TAR" -tzf "$pkg" 2>/dev/null)
    mapfile -t types < <("$_DAPK_TAR" -tvzf "$pkg" 2>/dev/null | cut -c1)
    n=${#names[@]}
    (( n > 0 && n == ${#types[@]} )) || { DAPK_PACK_ERROR="not a readable .$DABT_PKG_EXT (gzip tar) archive"; return 1; }
    first="${names[0]}"
    [[ "$first" == */MANIFEST && "${first%/MANIFEST}" != */* ]] || { DAPK_PACK_ERROR="first member must be <root>/MANIFEST (got '$first')"; return 1; }
    root="${first%/MANIFEST}"
    [[ "$root" =~ ^[a-z0-9][a-z0-9._+-]*$ ]] || { DAPK_PACK_ERROR="invalid package root '$root'"; return 1; }
    for i in "${!names[@]}"; do
        name="${names[i]}"; type="${types[i]}"
        [[ "$name" == "$root" || "$name" == "$root/"* ]] || { DAPK_PACK_ERROR="member outside the package root: $name"; return 1; }
        [[ "$name" != /* && "/$name/" != *"/../"* ]] || { DAPK_PACK_ERROR="unsafe member path: $name"; return 1; }
        [[ "$type" == - || "$type" == d ]] || { DAPK_PACK_ERROR="member '$name' is not a regular file or directory (links and devices are not allowed)"; return 1; }
    done
...
dapk.pack.manifest() {
    _dapk.pack.tar || return 1
    local first; first="$("$_DAPK_TAR" -tzf "$1" 2>/dev/null | head -n1)"
...
dapk.pack.file_in() {
    _dapk.pack.tar || return 1
    local first root; first="$("$_DAPK_TAR" -tzf "$1" 2>/dev/null | head -n1)"; root="${first%/MANIFEST}"
...
dapk.pack.extract() {
    ...
    [[ -f "$dest/$DAPK_PACK_ROOT/MANIFEST" ]] || { DAPK_PACK_ERROR="extraction failed"; return 1; }
