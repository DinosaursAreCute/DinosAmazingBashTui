...
dapk.ui.init() {
	...
	if ((tty)); then
		local _r _c
		read -r _r _c < <(stty size 2>/dev/null </dev/tty) && [[ "$_c" =~ ^[0-9]+$ ]] && DAPK_UI_COLS="$_c"
	fi
...
dapk.ui.step() {
	...
	if [[ -n "${GITHUB_ACTIONS:-}" ]]; then
		printf '::group::[%d/%d] %s\n' "$DAPK_UI_STEP" "$DAPK_UI_STEPS" "$1" >&2
		DAPK_UI_GROUP=1
	else printf '\n%s[%d/%d]%s %s%s%s\n' "$BLUE" "$DAPK_UI_STEP" "$DAPK_UI_STEPS" "$R" "$B" "$1" "$R" >&2; fi
}
dapk.ui.ok() {
	_dapk.ui.log INFO "ok: $1"
	((DAPK_UI_QUIET)) || printf '  %s✓%s %s\n' "$GRN" "$R" "$1" >&2
}
dapk.ui.info() {
	_dapk.ui.log INFO "$1"
	((DAPK_UI_QUIET)) || printf '  %s\n' "$1" >&2
}
dapk.ui.kv() {
	_dapk.ui.log INFO "$1: $2"
	((DAPK_UI_QUIET)) || printf '  %s%-10s%s %s\n' "$D" "$1" "$R" "$2" >&2
}
dapk.ui.debug() {
	_dapk.ui.log DEBUG "$1"
	((DAPK_UI_VERBOSE)) && printf '  %s%s%s\n' "$D" "$1" "$R" >&2
	return 0
}
dapk.ui.warn() {
	DAPK_UI_WARNINGS+=("$1")
...
_dapk.ui.paint() { # PCT
	local bar pct="$1"
	_dapk.ui.prefix 0
	if ((DAPK_UI_COLS < 60)); then
		printf '\r\e[K%s %s(%s)%s %3d%%' "$_PFX" "$_G250" "$_PG_LABEL" "$R" "$pct" >&2
	else
		dapk.ui.bar "$pct" 30 bar
		printf '\r\e[K%s %s(%s)%s  %s  %s%3d%%%s' "$_PFX" "$_G250" "$_PG_LABEL" "$R" "$bar" "$_G250" "$pct" "$R" >&2
	fi
}
...
dapk.ui.progress_tick() {
	...
	local pct=100
	((_PG_TOTAL > 0)) && pct=$((100 * _PG_CUR / _PG_TOTAL))
	((pct > 100)) && pct=100
...
_dapk.ui.sleep() {
	...
	read -r -t "$1" -u "$_DAPK_UI_FD" _ 2>/dev/null
	return 0
}
...
dapk.ui.run() {
	local label="$1" out rc off=0 pid
	shift
	[[ -n "$_DAPK_UI_TMP" ]] || _DAPK_UI_TMP="$(mktemp -d "${TMPDIR:-/tmp}/dapk_ui.XXXXXX")" || return 1
	out="$_DAPK_UI_TMP/run.out"
	: >"$out"
	...
	if ((DAPK_UI_PROGRESS && ! DAPK_UI_QUIET)); then
		...
		while kill -0 "$pid" 2>/dev/null; do
			_dapk.ui.prefix "$off"
			printf '\r\e[K%s %s(%s)%s' "$_PFX" "$_G250" "$label" "$R" >&2
			off=$(((off + 1) % 4))
			_dapk.ui.sleep 0.4
		done
		wait "$pid"
	...
	if ((rc)); then
		dapk.ui.err "$label failed (exit $rc)"
		((DAPK_UI_VERBOSE)) || tail -n 5 "$out" | sed 's/^/      /' >&2
	else dapk.ui.ok "$label"; fi
	return $rc
}
