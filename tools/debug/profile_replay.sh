...
ROOT="$(cd "$(dirname "$0")/../.." && pwd -P)"; DIR="$ROOT/lib"; PAGES_DIR="$ROOT/share/demo"
...
for p in "$@"; do
    ...
    _TUI_MARKUP_DIR="${f%/*}"; _TUI_MARKUP_FILE="$f"
    t0=${EPOCHREALTIME/[.,]/}; tui.cache.valid "$f"; t1=${EPOCHREALTIME/[.,]/}
    declare -A tot=() cnt=()
    while IFS= read -r cmd; do
        [[ -z "$cmd" ]] && continue
        w="${cmd%% *}"; w="${w//\'/}"
        a=${EPOCHREALTIME/[.,]/}; eval "$cmd" >/dev/null 2>&1; b=${EPOCHREALTIME/[.,]/}
        (( tot[$w] += b - a, cnt[$w]++ ))
