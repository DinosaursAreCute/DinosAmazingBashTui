...
"""screenshots.py - PNG screenshot of every DABT demo page under every theme (headless, no terminal needed).
  bin/debug/screenshots.py [-o OUTDIR] [-r ROWSxCOLS] [-t THEME,...] [-p PAGE|PAGE@TAB|PAGE@*,...] [--settle SEC] [-j JOBS] [--font PATH] [--size PX]
Runs screenshot_driver.sh once per theme (frames are dumped via tui.render), then rasterises the ANSI frame with a
small built-in VT emulator + Pillow. Output: OUTDIR/<ROWSxCOLS>/<theme>/<page>.png (-r takes a comma list, default 45x150,70x280) (default OUTDIR: bin/debug/screenshots/).
...
PAGES = os.path.normpath(os.path.join(HERE, '..', '..', 'config', 'DABT_demo'))
