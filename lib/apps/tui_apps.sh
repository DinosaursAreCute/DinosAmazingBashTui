...
tui.apps.meta_load() {
	...
	TUI_APPS_ERROR=""
	[[ -f "$f" ]] || {
		TUI_APPS_ERROR="no .dabt.metadata in $dir"
		return 1
	}
	...
	[[ -n "$name" ]] || {
		TUI_APPS_ERROR=".dabt.metadata: 'name' is missing"
		return 1
	}
	_tui_apps.valid_name "$name" || {
		TUI_APPS_ERROR=".dabt.metadata: invalid name '$name' (a-z 0-9 . _ -, not 'dabt')"
		return 1
	}
	[[ -n "$entry" ]] || {
		TUI_APPS_ERROR=".dabt.metadata: 'entry' is missing"
		return 1
	}
	[[ "$entry" != /* && "$entry" != *..* && -f "$dir/$entry" ]] || {
		TUI_APPS_ERROR=".dabt.metadata: entry '$entry' is not a file inside the app"
		return 1
	}
	case "${own,,}" in yes | true | 1) TUI_APP_META[own_dir]=yes ;; no | false | 0 | "") TUI_APP_META[own_dir]=no ;; *)
		TUI_APPS_ERROR=".dabt.metadata: own_dir must be yes or no"
		return 1
		;;
	esac
	for x in install_hook uninstall_hook; do
		[[ -z "${TUI_APP_META[$x]:-}" ]] && continue
		[[ "${TUI_APP_META[$x]}" != /* && "${TUI_APP_META[$x]}" != *..* && -f "$dir/${TUI_APP_META[$x]}" ]] || {
			TUI_APPS_ERROR=".dabt.metadata: $x '${TUI_APP_META[$x]}' is not a file inside the app"
			return 1
		}
	done
	for x in version min_dabt max_dabt; do
		[[ -z "${TUI_APP_META[$x]:-}" ]] && continue
		_tui_apps.valid_ver "${TUI_APP_META[$x]}" || {
			TUI_APPS_ERROR=".dabt.metadata: $x '${TUI_APP_META[$x]}' is not a version number"
			return 1
		}
	...
	if [[ -n "${TUI_APP_META[min_dabt]:-}" && -n "${TUI_APP_META[max_dabt]:-}" ]] && tui.version.newer "${TUI_APP_META[min_dabt]}" "${TUI_APP_META[max_dabt]}"; then
		TUI_APPS_ERROR=".dabt.metadata: min_dabt is higher than max_dabt"
		return 1
	fi
	return 0
}
...
tui.apps.compat() {
	...
	TUI_APPS_ERROR=""
	if [[ -n "$mn" ]] && tui.version.newer "$mn" "$TUI_VERSION"; then
		TUI_APPS_ERROR="needs DABT $mn or newer (this is $TUI_VERSION): run  dabt update"
		return 1
	fi
	if [[ -n "$mx" ]] && tui.version.newer "$TUI_VERSION" "$mx"; then
		TUI_APPS_ERROR="needs DABT $mx or older (this is $TUI_VERSION)"
		return 1
	fi
	return 0
}
...
_tui_apps.lookup() { # NAME -> A_* ; rc 1 when not installed
	...
	while IFS='|' read -r n v d e k w s; do
		[[ "$n" == "$1" ]] || continue
		A_NAME="$n" A_VERSION="$v" A_DIR="$d" A_ENTRY="$e" A_KIND="$k" A_WHEN="$w" A_SOURCE="$s"
		return 0
	done <"$TUI_APPS_LIST"
	return 1
}
...
_tui_apps.fetch() { # SRC TMPDIR -> _APP_SRC_DIR _APP_ORIGIN
	...
	if [[ -d "$src" ]]; then
		_APP_SRC_DIR="$(cd -P "$src" && pwd -P)"
		_APP_ORIGIN="$_APP_SRC_DIR"
	elif _tui_apps.is_url "$src"; then
		command -v git >/dev/null 2>&1 || {
			_tui_apps.err "git is needed to install from a URL"
			return 1
		}
		url="$src"
		[[ "$src" == *'#'* ]] && {
		...
		git clone -q --depth 1 ${ref:+--branch "$ref"} "$url" "$tmp/src" 2>"$tmp/git.log" ||
			{
				_tui_apps.err "git clone failed: $(tail -n1 "$tmp/git.log")"
				return 1
			}
		_APP_SRC_DIR="$tmp/src"
		_APP_ORIGIN="$src"
	else
		_tui_apps.err "'$src' is neither a folder nor a git URL"
		return 1
	fi
}
...
_tui_apps.copy() {
	...
	command -v tar >/dev/null 2>&1 || {
		_tui_apps.err "tar is not installed"
		return 1
	}
	_tui_apps.managed "$dest" || {
		_tui_apps.err "refusing to write to $dest"
		return 1
	}
	...
	(
		...
		rm -rf "$tmp"
		_tui_apps.err "copy to $dest failed"
		return 1
	}
	(
		shopt -s nullglob dotglob
		for f in "$tmp"/*; do cp -a "$f" "$dest/"; done
	)
	rm -rf "$tmp"
}
...
_tui_apps.hook() {
	...
	tui.apps.meta_load "$dir" 2>/dev/null
	h="${TUI_APP_META[$key]:-}"
	if [[ -z "$h" ]]; then
		printf 'dabt app: warning: %s has no %s in .dabt.metadata (nothing to run on %s)\n' "$name" "$key" "$event" >&2
		return 0
	fi
	printf '  running %s: %s\n' "$key" "$h"
...
tui.apps.install() {
	...
	while (($#)); do
		case "$1" in
			--force | -f) force=1 ;; --strict) strict=1 ;; --no-scan) noscan=1 ;; --name)
				name="$2"
				shift
				;;
			--entry)
			...
			--install-path)
				...
				;;
			-*)
				_tui_apps.err "install: unknown option $1"
				return 2
				;;
			*) [[ -z "$src" ]] && src="$1" || {
				_tui_apps.err "install: one source only"
				return 2
			} ;;
		esac
		shift
	...
	[[ -n "$src" ]] || {
		_tui_apps.err "install: give a folder or a git URL"
		return 2
	}
	tmp="$(mktemp -d)" || return 1
	...
	dir="$_APP_SRC_DIR"
	[[ -n "$origin" ]] && _APP_ORIGIN="$origin"
	if ((! noscan)); then
		tui.scan.run_spin "$dir" "Scanning incoming files for vulnerabilities"
		tui.scan.print
		if ((strict && TUI_SCAN_HIGH && ! force)); then
			_tui_apps.err "scan found $TUI_SCAN_HIGH high-risk issue(s): not installed (--force installs anyway)"
			return 1
		fi
	...
	if tui.apps.meta_load "$dir"; then
		name="${TUI_APP_META[name]}"
		entry="${TUI_APP_META[entry]}"
		own="${TUI_APP_META[own_dir]}"
		version="${TUI_APP_META[version]}"
		if ! tui.apps.compat; then
			((force)) || {
				_tui_apps.err "$name: $TUI_APPS_ERROR (--force installs it anyway)"
				return 1
			}
		...
		((force)) || {
			_tui_apps.err "$TUI_APPS_ERROR (--force installs it anyway; run it then with: dabt app run --entry PATH NAME)"
			return 1
		}
		...
		[[ -z "$entry" || ("$entry" != /* && "$entry" != *..* && -f "$dir/$entry") ]] || {
			_tui_apps.err "--entry '$entry' is not a file inside the app"
			return 1
		}
	fi
	_tui_apps.valid_name "$name" || {
		_tui_apps.err "invalid app name '$name' (use --name; a-z 0-9 . _ -, not 'dabt')"
		return 1
	}

	if [[ -n "$ipath" ]]; then
		[[ "$ipath" == /* && "$ipath" != *..* && "$ipath" != / && "$ipath" != "$HOME" ]] || {
			_tui_apps.err "--install-path must be an absolute folder path (not / or your home)"
			return 2
		}
		dest="${ipath%/}"
		_TUI_APPS_CUSTOM_DEST="$dest"
	elif [[ "$own" == yes ]]; then dest="$TUI_APPS_OWN_ROOT/$name"; else dest="$TUI_APPS_DIR/$name"; fi
	if _tui_apps.lookup "$name"; then
		updating=1
		prev="$A_DIR"
	elif [[ ("$own" == yes || -n "$ipath") && -e "$dest" ]] && [[ -n "$(ls -A "$dest" 2>/dev/null)" ]] && ((! force)); then
		_tui_apps.err "$dest already exists (--force replaces it)"
		return 1
	fi

	_tui_apps.copy "$dir" "$dest" || {
		_TUI_APPS_CUSTOM_DEST=""
		return 1
	}
...
tui.apps.update() {
	...
	while (($#)); do
		case "$1" in --path)
			...
			;;
		-*)
			_tui_apps.err "update: unknown option $1"
			return 2
			;;
		*) names+=("$1") ;; esac
		shift
	...
	if [[ -n "$path" ]]; then # update from a local folder or .dapk instead of the recorded source (works offline)
		((${#names[@]} == 1)) || {
			_tui_apps.err "update --path: give exactly one app name"
			return 2
		}
		_tui_apps.lookup "${names[0]}" || {
			_tui_apps.err "${names[0]} is not installed"
			return 1
		}
		[[ "$A_KIND" == builtin ]] && {
			_tui_apps.err "$A_NAME ships with DABT: update it with  dabt update --path PATH"
			return 1
		}
		local args=("$path")
		[[ "$A_KIND" == forced ]] && args+=(--force --name "$A_NAME" ${A_ENTRY:+--entry "$A_ENTRY"})
		tui.apps.install "${args[@]}"
		return $?
	fi
	local n rc=0 args
	if ((! ${#names[@]})); then
	...
	for n in "${names[@]}"; do
		_tui_apps.lookup "$n" || {
			_tui_apps.err "$n is not installed"
			rc=1
			continue
		}
		[[ "$A_KIND" == builtin ]] && {
			echo "$n ships with DABT: it is updated by  dabt update"
			continue
		}
		args=("$A_SOURCE")
		[[ "$A_KIND" == forced ]] && args+=(--force --name "$A_NAME" ${A_ENTRY:+--entry "$A_ENTRY"})
...
tui.apps.remove() {
	...
	for a in "$@"; do case "$a" in --yes | -y) yes=1 ;; --purge) purge=1 ;; -*)
		_tui_apps.err "remove: unknown option $a"
		return 2
		;;
	*) name="$a" ;; esac done
	[[ -n "$name" ]] || {
		_tui_apps.err "remove: give an app name"
		return 2
	}
	_tui_apps.lookup "$name" || {
		_tui_apps.err "$name is not installed"
		return 1
	}
	[[ "$A_KIND" == builtin ]] && {
		_tui_apps.err "$name ships with DABT and cannot be removed"
		return 1
	}
	...
	if ((purge)); then
		_tui_apps.managed "$A_DIR" && rm -rf -- "$A_DIR"
		_tui_apps.managed "$TUI_APPS_DIR/$name" && rm -rf -- "$TUI_APPS_DIR/$name"
	else _tui_apps.wipe "$A_DIR"; fi
	_tui_apps.unregister "$name"
	rm -f "$TUI_HOME/apps.deps/$name"
	echo "removed $name"
}
...
tui.apps.info() {
	_tui_apps.lookup "${1:-}" || {
		_tui_apps.err "${1:-?} is not installed"
		return 1
	}
	local x
...
tui.apps.run() {
	...
	while (($#)); do
		case "$1" in --entry)
			...
			;;
		--force | -f) force=1 ;; -*)
			_tui_apps.err "run: unknown option $1"
			return 2
			;;
		*)
			target="$1"
	...
	[[ -n "$target" ]] || {
		_tui_apps.err "run: give an app name or the path to its entry script"
		return 2
	}
	if _tui_apps.lookup "$target"; then
		name="$A_NAME"
		dir="$A_DIR"
		[[ -n "$entry" ]] || entry="$A_ENTRY"
		[[ -n "$entry" ]] || {
			_tui_apps.err "$name has no entry script recorded (no .dabt.metadata): dabt app run --entry PATH $name"
			return 1
		}
		[[ "$entry" == /* ]] && file="$entry" || file="$dir/$entry"
	elif [[ -f "$target" ]]; then
		file="$(cd -P "$(dirname "$target")" && pwd -P)/${target##*/}"
		dir="${file%/*}"
	elif [[ -d "$target" ]]; then
		dir="$(cd -P "$target" && pwd -P)"
		if [[ -n "$entry" ]]; then
			[[ "$entry" != /* && "$entry" != *..* && -f "$dir/$entry" ]] || {
				_tui_apps.err "--entry '$entry' is not a file inside $dir"
				return 1
			}
			file="$dir/$entry"
		else
			tui.apps.meta_load "$dir" || {
				_tui_apps.err "$TUI_APPS_ERROR (pass the entry script itself, or --entry PATH)"
				return 1
			}
			file="$dir/${TUI_APP_META[entry]}"
		fi
	else
		_tui_apps.err "'$target' is not an installed app, an entry script or an app folder (see: dabt app list)"
		return 1
	fi
	[[ -f "$file" ]] || {
		_tui_apps.err "entry script not found: $file"
		return 1
	}
	if tui.apps.meta_load "$dir" 2>/dev/null; then
		[[ -n "$name" ]] || name="${TUI_APP_META[name]}"
		tui.apps.compat || { ((force)) && printf 'dabt app: warning: %s\n' "$TUI_APPS_ERROR" >&2 || {
			_tui_apps.err "$name: $TUI_APPS_ERROR (--force runs it anyway)"
			return 1
		}; }
	fi
	if [[ -z "$name" ]]; then
