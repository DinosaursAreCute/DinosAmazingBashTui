...
tui.footer.set() {
    ...
    _TUI_FOOTER_STAMP=""
    if (( ! _TUI_FOOTER_ON )); then _TUI_FOOTER_ON=1; tui.overlay.add _tui_footer.draw; fi
    return 0
}
tui.footer.show() { tui.footer.set "$1"; if (( _TUI_RUNNING )); then _tui._root_h; tui.relayout; fi; }
tui.footer.add()  { tui.footer.show "${_TUI_FOOTER_ITEMS:-$TUI_FOOTER_DEFAULT};$1|$2${3:+|$3}"; }
tui.footer.hide() {
    (( _TUI_FOOTER_ON )) || return 0
    _TUI_FOOTER_ON=0; tui.overlay.remove _tui_footer.draw
    if (( _TUI_RUNNING )); then _tui._root_h; tui.relayout; fi
}
# tui.reset_ui: the footer belongs to the page that declared it
_tui_footer.reset() { _TUI_FOOTER_ON=0; _TUI_FOOTER_ITEMS=""; _TUI_FOOTER_STAMP=""; tui.overlay.remove _tui_footer.draw; }
...
_tui_footer.build() {
    ...
    tui.class.sgr footer;       s_bar="${TUI_SGR:-$'\e[0;37;48;2;30;34;52m'}"
    tui.class.sgr footer_key;   s_key="${TUI_SGR:-$'\e[1;30;48;2;97;175;239m'}"
    tui.class.sgr footer_label; s_lbl="${TUI_SGR:-$s_bar}"
    IFS=';' read -ra items <<< "$_TUI_FOOTER_ITEMS"
    for item in "${items[@]}"; do
        IFS='|' read -r key label when <<< "$item"
        [[ -n "$when" ]] && ! "$when" && continue
        if [[ "$key" == @* ]]; then key="${_REVK[${key#@}]:-}"; [[ -z "$key" ]] && continue; fi
        seg=" ${key} ${label} "
        (( plain + ${#seg} + 1 > w )) && break
        out+="${s_key} ${key} ${s_lbl} ${label} ${s_bar} "
        (( plain += ${#seg} + 1 ))
    done
    local pad; printf -v pad '%*s' "$(( w - 1 - plain > 0 ? w - 1 - plain : 0 ))" ''
...
_tui_footer.draw() {
    ...
    if [[ "$stamp" != "$_TUI_FOOTER_STAMP" ]]; then _TUI_FOOTER_STAMP="$stamp"; _tui_footer.build; fi
