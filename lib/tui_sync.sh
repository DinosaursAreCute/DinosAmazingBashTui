...
_tui_sync.sha() {   # FILE -> _SHA ("" when unreadable)
    ...
    elif command -v openssl >/dev/null 2>&1; then out="$(openssl dgst -sha256 "$1")"; out="${out##* }"
    else out="$(cksum "$1")"; out="${out%% *}"; fi
    _SHA="${out%% *}"
}
...
tui.sync.plan() {
    ...
    TUI_SYNC_ADD=(); TUI_SYNC_UPDATE=(); TUI_SYNC_SAME=(); TUI_SYNC_KEEP=(); TUI_SYNC_CONFLICT=(); TUI_SYNC_REMOVE=(); TUI_SYNC_ORPHAN=()
    _tui_sync.manifest_load "$conf"
    local -A inrelease=()
    while IFS= read -r rel; do
        [[ -n "$rel" ]] || continue
        inrelease[$rel]=1
        _tui_sync.sha "$(_tui_sync.config_src "$src" "$rel")"; new="$_SHA"
        if [[ ! -f "$conf/$rel" ]]; then TUI_SYNC_ADD+=("$rel"); continue; fi
        _tui_sync.sha "$conf/$rel"; cur="$_SHA"
        base="${_TUI_SYNC_BASE[$rel]:-}"
        if   [[ "$cur" == "$new" ]]; then TUI_SYNC_SAME+=("$rel")
        elif [[ -n "$base" && "$cur" == "$base" ]]; then TUI_SYNC_UPDATE+=("$rel")
        elif [[ -n "$base" && "$new" == "$base" ]]; then TUI_SYNC_KEEP+=("$rel")
...
_tui_sync.backup() {   # CONFIG REL  (copies CONFIG/REL into the backup folder, creating it on first use)
    [[ -f "$1/$2" ]] || return 0
    if [[ -z "$TUI_SYNC_BACKUP" ]]; then _tui_sync.stamp; TUI_SYNC_BACKUP="$1/backups/$_STAMP"; fi
    _tui_sync.dirof "$2"; mkdir -p "$TUI_SYNC_BACKUP/$_D" && cp -p "$1/$2" "$TUI_SYNC_BACKUP/$2"
}
...
tui.sync.apply() {
    ...
    TUI_SYNC_BACKUP=""; TUI_SYNC_COUNT=([add]=0 [update]=0 [same]=0 [keep]=0 [override]=0 [skip]=0 [new]=0 [remove]=0 [orphan]=0)
...
_tui_sync.program_list() {   # DIR -> relative paths of the program files below DIR
    ...
    for t in "${_TUI_SYNC_PROGRAM_TOP[@]}"; do
        if [[ -d "$1/$t" ]]; then while IFS= read -r f; do printf '%s/%s\n' "$t" "$f"; done < <(_tui_sync.files "$1/$t")
...
tui.sync.program_plan() {
    ...
    TUI_SYNC_P_ADD=(); TUI_SYNC_P_UPDATE=(); TUI_SYNC_P_SAME=(); TUI_SYNC_P_REMOVE=()
    local -A inrelease=()
    while IFS= read -r rel; do
        inrelease[$rel]=1
        if [[ ! -f "$prefix/$rel" ]]; then TUI_SYNC_P_ADD+=("$rel"); continue; fi
        _tui_sync.sha "$src/$rel"; a="$_SHA"; _tui_sync.sha "$prefix/$rel"; b="$_SHA"
...
tui.sync.program_apply() {   # SRC PREFIX [BACKUP_DIR]
    ...
    for rel in "${TUI_SYNC_P_UPDATE[@]}" "${TUI_SYNC_P_REMOVE[@]}"; do
        [[ -n "$bk" && -f "$prefix/$rel" ]] && { _tui_sync.dirof "$rel"; mkdir -p "$bk/program/$_D" && cp -p "$prefix/$rel" "$bk/program/$rel"; }
    done
    for rel in "${TUI_SYNC_P_ADD[@]}" "${TUI_SYNC_P_UPDATE[@]}"; do _tui_sync.dirof "$rel"; mkdir -p "$prefix/$_D" && cp -p "$src/$rel" "$prefix/$rel"; done
...
tui.sync.report() {
    local rel out=""
    _r() { local title="$1"; shift; (( $# )) || return 0; out+="$title ($#)"$'\n'; for rel in "$@"; do out+="    $rel"$'\n'; done; }
