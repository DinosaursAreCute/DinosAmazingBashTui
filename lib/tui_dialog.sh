...
_tui_dialog.wrap() {
    ...
    while IFS= read -r para || [[ -n "$para" ]]; do
        ...
        for word in $para; do
            while (( ${#word} > w )); do
                [[ -n "$line" ]] && { _WRAP+=("$line"); line=""; }
                _WRAP+=("${word:0:w}"); word="${word:w}"
            done
            if [[ -z "$line" ]]; then line="$word"
            elif (( ${#line} + 1 + ${#word} <= w )); then line+=" $word"
            else _WRAP+=("$line"); line="$word"; fi
        done
        _WRAP+=("$line")
    done <<< "$text"
}
...
_tui_dialog.call() {
    ...
    read -ra words <<< "$cmd"
    (( ${#words[@]} )) || return 0
    if declare -F "${words[0]}" >/dev/null; then "${words[@]}" "$@"
...
_tui_dialog.styles() {
    _tui_dialog.sgr dialog        $'\e[0;97;48;2;30;34;52m';  _DLG_SGR_BOX="$_SG"
    _tui_dialog.sgr dialog_title  $'\e[1;97;48;2;30;34;52m';  _DLG_SGR_TITLE="$_SG"
    _tui_dialog.sgr dialog_btn    $'\e[0;97;48;2;62;68;92m';  _DLG_SGR_BTN="$_SG"
    _tui_dialog.sgr dialog_btn_sel $'\e[1;30;48;2;97;175;239m'; _DLG_SGR_SEL="$_SG"
    _tui_dialog.sgr dialog_danger $'\e[1;97;48;2;176;0;32m';  _DLG_SGR_DANGER="$_SG"
    _tui_dialog.sgr dialog_dim    $'\e[2;37;48;2;30;34;52m';  _DLG_SGR_DIM="$_SG"
    _tui_dialog.sgr dialog_error  $'\e[1;91;48;2;30;34;52m';  _DLG_SGR_ERR="$_SG"
}
...
tui.view() {
    _tui_dialog.parse "$@"
    _DLG_KIND=view; _DLG_TITLE="${_POS[0]:-View}"; _DLG_ON_CANCEL="${_DLG_ON_CANCEL:-}"
    _DLG_ITEMS=(); local l
    while IFS= read -r l || [[ -n "$l" ]]; do _DLG_ITEMS+=("$l"); done <<< "${_POS[1]:-}"
    _DLG_BTN=(); _DLG_BTN_ID=(); _DLG_SEL=0; _DLG_TOP=0
    _tui_dialog.open
}

tui.dialog.active() { tui.modal.active dialog; }
tui.dialog.close()  { tui.modal.active dialog && tui.modal.close; return 0; }

# _tui_dialog.parse ARGS... : flags into _DLG_*, positionals into _POS
_tui_dialog.parse() {
    _POS=(); _DLG_TITLE=""; _DLG_WIDTH=0; _DLG_DANGER=0; _DLG_DEFAULT=""
    _DLG_VAL=""; _DLG_PH=""; _DLG_VALIDATE=""; _DLG_ON_CANCEL=""; _DLG_MSG=""
    _DLG_YES_L="Yes"; _DLG_NO_L="No"; _DLG_OK_L="OK"; _DLG_SEL0=0
    while (( $# )); do
        case "$1" in
            --title)       _DLG_TITLE="$2"; shift ;;
            --width)       _DLG_WIDTH="$2"; shift ;;
            --ok)          _DLG_OK_L="$2"; shift ;;
            --yes)         _DLG_YES_L="$2"; shift ;;
            --no)          _DLG_NO_L="$2"; shift ;;
            --danger)      _DLG_DANGER=1 ;;
            --default)     _DLG_DEFAULT="$2"; shift ;;
            --value)       _DLG_VAL="$2"; shift ;;
            --placeholder) _DLG_PH="$2"; shift ;;
            --validate)    _DLG_VALIDATE="$2"; shift ;;
            --cancel)      _DLG_ON_CANCEL="$2"; shift ;;
            --message)     _DLG_MSG="$2"; shift ;;
            --selected)    _DLG_SEL0="$2"; shift ;;
            *)             _POS+=("$1") ;;
        esac
        shift
    done
...
tui.confirm() {
    _tui_dialog.parse "$@"
    _DLG_KIND=confirm; _DLG_MSG="${_POS[0]:-Are you sure?}"; _DLG_ON_YES="${_POS[1]:-}"; _DLG_ON_NO="${_POS[2]:-}"
    [[ -n "$_DLG_TITLE" ]] || { (( _DLG_DANGER )) && _DLG_TITLE="Warning" || _DLG_TITLE="Confirm"; }
    _DLG_BTN=("$_DLG_YES_L" "$_DLG_NO_L"); _DLG_BTN_ID=(yes no)
    _DLG_SEL=0; [[ "$_DLG_DEFAULT" == no ]] && _DLG_SEL=1
...
tui.message() {
    _tui_dialog.parse "$@"
    _DLG_KIND=message; _DLG_MSG="${_POS[0]:-}"; _DLG_ON_YES="${_POS[1]:-}"; _DLG_ON_NO=""
    [[ -n "$_DLG_TITLE" ]] || _DLG_TITLE="Message"
    _DLG_BTN=("$_DLG_OK_L"); _DLG_BTN_ID=(ok); _DLG_SEL=0
    _tui_dialog.open
}

tui.prompt() {
    _tui_dialog.parse "$@"
    _DLG_KIND=prompt; _DLG_MSG="${_POS[0]:-}"; _DLG_ON_SUBMIT="${_POS[1]:-}"
    [[ -n "$_DLG_TITLE" ]] || _DLG_TITLE="Input"
    _DLG_BTN=("$_DLG_OK_L" "Cancel"); _DLG_BTN_ID=(submit cancel); _DLG_SEL=0
    _TUI_W_TYPE[__dlg]=input; _TUI_W_VALUE[__dlg]="$_DLG_VAL"; _TXC[__dlg]=${#_DLG_VAL}; _TXA[__dlg]=-1; _TXS[__dlg]=0
    _tui_dialog.open
}

tui.choose() {
    _tui_dialog.parse "$@"
    _DLG_KIND=choose; _DLG_TITLE="${_POS[0]:-Choose}"; _DLG_ON_SUBMIT="${_POS[1]:-}"
    _DLG_ITEMS=("${_POS[@]:2}")
    (( ${#_DLG_ITEMS[@]} )) || return 1
    _DLG_BTN=(); _DLG_BTN_ID=()
    _DLG_SEL=$_DLG_SEL0; (( _DLG_SEL >= ${#_DLG_ITEMS[@]} )) && _DLG_SEL=0
    _tui_dialog.open
}
...
_tui_dialog.key() {
    ...
    case "$_DLG_KIND" in
        confirm|message)
            case "$k" in
                y) [[ "$_DLG_KIND" == confirm ]] && { _tui_dialog.press 0; return; } ;;
                n) [[ "$_DLG_KIND" == confirm ]] && { _tui_dialog.press 1; return; } ;;
                esc|ctrl+g) if [[ "$_DLG_KIND" == confirm ]]; then _tui_dialog.press 1; else _tui_dialog.press 0; fi; return ;;
                enter|space) _tui_dialog.press "$_DLG_SEL"; return ;;
                left|shift+tab|up|h) (( _DLG_SEL > 0 )) && (( _DLG_SEL-- )) ;;
                right|tab|down|l)    (( _DLG_SEL < ${#_DLG_BTN[@]} - 1 )) && (( _DLG_SEL++ )) ;;
            esac ;;
        prompt)
            case "$k" in
                esc|ctrl+g) _tui_dialog.press 1; return ;;
                enter)      _DLG_VAL="${_TUI_W_VALUE[__dlg]}"; _tui_dialog.submit; return ;;
                paste)      tui.text.insert __dlg "$TUI_EVENT_PASTE" paste; _DLG_ERR="" ;;
                *)          _tui_text.consumes __dlg "$k" && { _tui_text.key __dlg "$k"; _DLG_ERR=""; } ;;
            esac
            _DLG_VAL="${_TUI_W_VALUE[__dlg]}" ;;
        view)
            n=${#_DLG_ITEMS[@]}
            case "$k" in
                esc|q|enter|ctrl+g) _tui_dialog.finish cancel "$_DLG_ON_CANCEL"; return ;;
            ...
            (( _DLG_TOP > n - _DLG_ROWS )) && _DLG_TOP=$(( n - _DLG_ROWS )); (( _DLG_TOP < 0 )) && _DLG_TOP=0 ;;
        choose)
            n=${#_DLG_ITEMS[@]}
            case "$k" in
                esc|ctrl+g|q) _tui_dialog.finish cancel "$_DLG_ON_CANCEL"; return ;;
                enter|space)  _tui_dialog.finish choose "$_DLG_ON_SUBMIT" "$_DLG_SEL" "${_DLG_ITEMS[_DLG_SEL]}"; return ;;
                up|k|ctrl+p)   (( _DLG_SEL > 0 )) && (( _DLG_SEL-- )) ;;
                down|j|ctrl+n) (( _DLG_SEL < n - 1 )) && (( _DLG_SEL++ )) ;;
                pgup)  (( _DLG_SEL -= _DLG_ROWS )); (( _DLG_SEL < 0 )) && _DLG_SEL=0 ;;
                pgdn)  (( _DLG_SEL += _DLG_ROWS )); (( _DLG_SEL >= n )) && _DLG_SEL=$(( n - 1 )) ;;
                home)  _DLG_SEL=0 ;;
                end)   _DLG_SEL=$(( n - 1 )) ;;
                [1-9]) if (( k <= n )); then _tui_dialog.finish choose "$_DLG_ON_SUBMIT" "$(( k - 1 ))" "${_DLG_ITEMS[k-1]}"; return; fi ;;
            esac ;;
    esac
    tui.modal.redraw
}
...
_tui_dialog.mouse() {
    ...
    case "$ev" in
        wheel:up)   if [[ "$_DLG_KIND" == view ]]; then (( _DLG_TOP > 0 )) && { (( _DLG_TOP -= 3 )); (( _DLG_TOP < 0 )) && _DLG_TOP=0; tui.modal.redraw; }
                    else [[ "$_DLG_KIND" == choose ]] && (( _DLG_SEL > 0 )) && { (( _DLG_SEL-- )); tui.modal.redraw; }; fi ;;
        wheel:down) if [[ "$_DLG_KIND" == view ]]; then (( _DLG_TOP < ${#_DLG_ITEMS[@]} - _DLG_ROWS )) && { (( _DLG_TOP += 3 )); tui.modal.redraw; }
                    else [[ "$_DLG_KIND" == choose ]] && (( _DLG_SEL < ${#_DLG_ITEMS[@]} - 1 )) && { (( _DLG_SEL++ )); tui.modal.redraw; }; fi ;;
        mouse:left)
            ...
            if (( y == _DLG_BROW )); then
                for (( i = 0; i < ${#_DLG_BTN[@]}; i++ )); do
                    (( x >= _DLG_BLO[i] && x <= _DLG_BHI[i] )) && { _tui_dialog.press "$i"; return; }
                done
            fi ;;
    esac
}
...
_tui_dialog.draw_view() {
    ...
    w=${_DLG_WIDTH:-0}
    if (( w <= 0 )); then w=20; for line in "${_DLG_ITEMS[@]}"; do (( ${#line} + 4 > w )) && w=$(( ${#line} + 4 )); done; fi
    (( w > _TUI_COLS - 2 )) && w=$(( _TUI_COLS - 2 )); (( w < 20 )) && w=20
    inner=$(( w - 2 ))
    local rows=$n; (( rows > _TUI_ROWS - 6 )) && rows=$(( _TUI_ROWS - 6 )); (( rows < 1 )) && rows=1
    _DLG_ROWS=$rows
    (( _DLG_TOP > n - rows )) && _DLG_TOP=$(( n - rows )); (( _DLG_TOP < 0 )) && _DLG_TOP=0
    h=$(( rows + 4 ))                                    # top, rows, blank footer line, bottom (+ 1 padding)
    x=$(( (_TUI_COLS - w) / 2 + 1 )); (( x < 1 )) && x=1
    y=$(( (_TUI_ROWS - h) / 2 )); (( y < 1 )) && y=1
    _DLG_X=$x; _DLG_Y=$y; _DLG_W=$w; _DLG_H=$h
...
_tui_dialog.draw() {
    ...
    [[ "$_DLG_KIND" == view ]] && { _tui_dialog.draw_view; return 0; }
    ...
    for line in "${msg[@]}"; do _tui_dialog.pad " $line" "$inner"; body+=("${_DLG_SGR_BOX}${_PADS}"); done
    ...
    if [[ "$_DLG_KIND" == prompt ]]; then
        ...
        if [[ -z "$_DLG_VAL" && -n "$_DLG_PH" ]]; then
            ...
            local pa=-1 pb=-1
            if _tui_text.sel __dlg; then pa=$(( _SA - s )); pb=$(( _SB - s )); (( pa < 0 )) && pa=0; (( pb > avail )) && pb=$avail; fi
            _tui_text.paint_v "${_DLG_VAL:s:avail}" "$pa" "$pb" "$(( c - s ))" "$avail" "$_DLG_SGR_BTN"
            line="${_DLG_SGR_BOX} ${_DLG_SGR_BTN} ${_PV} ${_DLG_SGR_BOX} "
        fi
        body+=("$line"); prompt_row=${#body[@]}
        if [[ -n "$_DLG_ERR" ]]; then _tui_dialog.pad " $_DLG_ERR" "$inner"; body+=("${_DLG_SGR_ERR}${_PADS}")
    ...
    if [[ "$_DLG_KIND" == choose ]]; then
        ...
        (( rows > n )) && rows=$n
        (( rows > _TUI_ROWS - 8 )) && rows=$(( _TUI_ROWS - 8 )); (( rows < 1 )) && rows=1
        _DLG_ROWS=$rows
    ...
    if (( ${#_DLG_BTN[@]} )); then
        ...
        (( total -= 2 ))
        local lead=$(( (inner - total) / 2 )); (( lead < 0 )) && lead=0
        printf -v sp '%*s' "$lead" ''
        ...
        for (( i = 0; i < ${#_DLG_BTN[@]}; i++ )); do
            ...
            _DLG_BLO[i]=$(( cx + 1 )); _DLG_BHI[i]=$(( cx + ${#lbl} ))      # x offsets inside the frame; made absolute below
            line+="${sgr}${lbl}${_DLG_SGR_BOX}  "
            (( cx += ${#lbl} + 2 ))
        done
        printf -v sp '%*s' "$(( inner - cx ))" ''; (( inner - cx < 0 )) && sp=""
        line+="$sp"
        body+=("$line"); btnrow=${#body[@]}
    fi
    ...
    y=$(( (_TUI_ROWS - h) / 2 )); (( y < 1 )) && y=1
    _DLG_X=$x; _DLG_Y=$y; _DLG_W=$w; _DLG_H=$h
    (( btnrow > 0 )) && { _DLG_BROW=$(( y + btnrow )); for (( i = 0; i < ${#_DLG_BLO[@]}; i++ )); do (( _DLG_BLO[i] += x )); (( _DLG_BHI[i] += x )); done; }
    (( list_first >= 0 )) && _DLG_LROW=$(( y + 1 + list_first ))

    local title=" $_DLG_TITLE "
    local tsg="$_DLG_SGR_TITLE"; (( _DLG_DANGER )) && [[ "$_DLG_KIND" == confirm ]] && tsg=$'\e[1;91;48;2;30;34;52m'
...
_tui_dialog.toast_styles() {
    _tui_dialog.sgr toast         $'\e[0;97;48;2;38;56;92m';  _TST_SGR_INFO="$_SG"
    _tui_dialog.sgr toast_success $'\e[0;30;48;2;51;204;102m'; _TST_SGR_SUCCESS="$_SG"
    _tui_dialog.sgr toast_warn    $'\e[0;30;48;2;236;201;75m'; _TST_SGR_WARN="$_SG"
    _tui_dialog.sgr toast_error   $'\e[1;97;48;2;176;0;32m';  _TST_SGR_ERROR="$_SG"
    _TST_STYLED=1
}
...
tui.notify() {
    ...
    while (( n >= TUI_TOAST_MAX )); do                                    # drop the oldest
        _TST_ID=("${_TST_ID[@]:1}"); _TST_MSG=("${_TST_MSG[@]:1}"); _TST_LVL=("${_TST_LVL[@]:1}"); _TST_EXP=("${_TST_EXP[@]:1}"); (( n-- ))
    done
    local id=$(( ++_TST_SEQ )) exp=0
    if [[ "$secs" != 0 ]]; then
        local ms; _tui_api._ms "$secs"; ms=$_TA_MS
        local now=${EPOCHREALTIME//[.,]/}; exp=$(( now / 1000 + ms ))
        tui.after "$secs" _tui_dialog.expire "toast_$id"
    fi
    _TST_ID+=("$id"); _TST_MSG+=("$msg"); _TST_LVL+=("$level"); _TST_EXP+=("$exp")
    TUI_NOTIFY_ID=$id
    if (( ! _TST_ON )); then _TST_ON=1; tui.overlay.add _tui_dialog.toast_draw; fi
    (( _TUI_RUNNING )) && _tui_overlay.draw_all
    return 0
}

tui.notify.position() {
    [[ -z "${1:-}" ]] && { printf '%s\n' "$TUI_TOAST_POSITION"; return 0; }
    case "$1" in
        bottom-right|bottom-left|bottom-center|top-right|top-left|top-center) TUI_TOAST_POSITION="$1" ;;
        *) echo "tui.notify.position: bottom-right|bottom-left|bottom-center|top-right|top-left|top-center" >&2; return 1 ;;
    esac
    (( _TUI_RUNNING && ${#_TST_ID[@]} )) && tui.relayout            # wipe the old spot, redraw at the new one
    return 0
}
tui.notify.seconds() {
    [[ -z "${1:-}" ]] && { printf '%s\n' "$TUI_TOAST_SECONDS"; return 0; }
...
tui.notify.clear() {
    ...
    for (( i = 0; i < ${#_TST_ID[@]}; i++ )); do
        if [[ -z "$id" || "${_TST_ID[i]}" == "$id" ]]; then
            ...
            nid+=("${_TST_ID[i]}"); nmsg+=("${_TST_MSG[i]}"); nlvl+=("${_TST_LVL[i]}"); nexp+=("${_TST_EXP[i]}")
        fi
    done
    _TST_ID=("${nid[@]}"); _TST_MSG=("${nmsg[@]}"); _TST_LVL=("${nlvl[@]}"); _TST_EXP=("${nexp[@]}")
    (( ${#_TST_ID[@]} )) || { _TST_ON=0; tui.overlay.remove _tui_dialog.toast_draw; }
...
_tui_dialog.reset() {
    ...
    for (( i = 0; i < n; i++ )); do
        ...
        left=$(( _TST_EXP[i] - now )); (( left < 100 )) && left=100
...
_tui_dialog.toast_draw() {
    ...
    for (( i = n - 1; i >= 0; i-- )); do                                 # newest at the edge, older ones stack inward
        case "${_TST_LVL[i]}" in
            success) sgr="$_TST_SGR_SUCCESS"; icon="✔" ;;
            warn)    sgr="$_TST_SGR_WARN";    icon="▲" ;;
            error)   sgr="$_TST_SGR_ERROR";   icon="✖" ;;
            *)       sgr="$_TST_SGR_INFO";    icon="●" ;;
        esac
        _tui_dialog.wrap "${_TST_MSG[i]}" "$(( inner - 2 ))"
        local -a lines=("${_WRAP[@]:0:3}")
        local h=$(( ${#lines[@]} + 2 )) top
        if (( atbottom )); then top=$(( y - h + 1 )); (( top < 1 )) && break
        else top=$y; (( top + h - 1 > _TUI_ROWS - _TUI_FOOTER_ON )) && break; fi
        printf -v line '%*s' "$(( w - 2 ))" ''
        out+="${sgr}"$'\e['"${top};${x}H┌${line// /─}┐"
        for (( j = 0; j < ${#lines[@]}; j++ )); do
            _tui_dialog.pad " ${lines[j]}" "$inner"
            if (( j == 0 )); then out+=$'\e['"$(( top + 1 + j ));${x}H│ ${icon}${_PADS:0:$(( inner - 1 ))} │"
