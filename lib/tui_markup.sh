...
_markup_expand() {
	...
	dir="$(cd "$(dirname "$file")" 2>/dev/null && pwd)" || {
		echo "tui.load: cannot resolve '$file'" >&2
		return 1
	}
	base="$(basename "$file")"
	key="${dir}/${base}"
	...
	while IFS= read -r raw_line || [[ -n "$raw_line" ]]; do
		...
		case "$tag" in
			tui)
				((closing)) && continue
				local visit
				visit="$(_markup_attr "$line" on_visit)"
			...
			input)
				...
				local iretain
				iretain="$(_markup_attr "$line" retain_input_on_submit)"
			...
			bind)
				...
				local bscope
				bscope="$(_markup_attr "$line" scope)"
				local -a bflags=(--page)
...
_tui_cache_relayout() {
	_tui._root_h
	_tui._layout root
} # _root_h: leave the last row to a <footer/>
