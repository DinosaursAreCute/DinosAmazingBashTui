...
_mon_refresh_mem() {
	...
	_mon_kb_to_gb_v "$used"
	used_gb="$_MG"
	_mon_kb_to_gb_v "$cache_buff"
	cache_gb="$_MG"
	_mon_kb_to_gb_v "$shown_free"
	free_gb="$_MG"
	_mon_kb_to_gb_v "$total"
	total_gb="$_MG"
	...
	local _out_b
	printf -v _out_b '%b' "$out"
	_mon_output_fit "mem_pane" "$_out_b"
}
...
_mon_refresh_load() {
	...
	local _out_b
	printf -v _out_b '%b' "$out"
	_mon_output_fit "load_pane" "$_out_b"
}
...
_mon_refresh_disk() {
	...
	local _out_b
	printf -v _out_b '%b' "$out"
	_mon_output_fit "disk_pane" "$_out_b"
}
