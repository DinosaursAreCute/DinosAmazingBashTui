...
#   bin/debug/count_forks.sh [-p PAGE] [-n RUNS] 'COMMAND ARGS' ['COMMAND ARGS' ...]
#   bin/debug/count_forks.sh -p components '_tui._handle_mouse "[<35;40;10M"' 'tui.render' 'tui.update lbl_status x'
...
DIR="$(cd "$(dirname "$0")/.." && pwd)"; PAGE=components; RUNS=3
while [[ "$1" == -* ]]; do case "$1" in -p) PAGE="$2"; shift 2 ;; -n) RUNS="$2"; shift 2 ;; *) break ;; esac; done
(( $# )) || { sed -n '2,12p' "$0" | sed 's/^# \?//'; exit 1; }
export XDG_CONFIG_HOME="$(mktemp -d)"; trap 'rm -rf "$XDG_CONFIG_HOME"' EXIT
cd "$DIR" && source ./tui.sh
_TUI_ROWS=45; _TUI_COLS=150
_TUI_P_ROW[root]=1; _TUI_P_COL[root]=1; _TUI_P_H[root]=45; _TUI_P_W[root]=150
tui.load "$DIR/../config/DABT_demo/$PAGE.xml" >/dev/null 2>&1
_TUI_RUNNING=1
