...
_tui_update.version_url() {
	if _tui_update.release; then
		printf '%s' "https://api.github.com/repos/$TUI_UPDATE_REPO/releases/latest"
...
_tui_update.archive_url() {
	if [[ -n "${TUI_UPDATE_ARCHIVE_URL:-}" ]]; then
		printf '%s' "$TUI_UPDATE_ARCHIVE_URL"
	elif [[ -n "$TUI_UPDATE_TAG" && "$TUI_UPDATE_CHANNEL" == release ]]; then
		printf '%s' "https://github.com/$TUI_UPDATE_REPO/archive/refs/tags/$TUI_UPDATE_TAG.tar.gz"
	else printf '%s' "https://github.com/$TUI_UPDATE_REPO/archive/refs/heads/$TUI_UPDATE_BRANCH.tar.gz"; fi
}
_tui_update.changelog_url() {
	local ref="$TUI_UPDATE_BRANCH"
	[[ -n "$TUI_UPDATE_TAG" && "$TUI_UPDATE_CHANNEL" == release ]] && ref="$TUI_UPDATE_TAG"
...
_tui_update.news_since() {
	local since="$1" tmp
	tmp="$(mktemp)" || return 1
	_tui_update.fetch "$(_tui_update.changelog_url)" "$tmp" || {
		rm -f "$tmp"
		return 1
	}
	_tui_update.news_since_file "$tmp" "$since"
	rm -f "$tmp"
}
...
_tui_update.fetch() {
	if command -v curl >/dev/null 2>&1; then
		curl -fsSL --max-time "$TUI_UPDATE_TIMEOUT" -o "$2" "$1" 2>/dev/null
	elif command -v wget >/dev/null 2>&1; then
		wget -q -T "$TUI_UPDATE_TIMEOUT" -O "$2" "$1" 2>/dev/null
	else
		TUI_UPDATE_ERROR="neither curl nor wget is installed"
		return 1
	fi
}
...
tui.update.check() {
	...
	TUI_UPDATE_ERROR=""
	tmp="$(mktemp)" || {
		TUI_UPDATE_ERROR="cannot create a temporary file"
		return 2
	}
	...
	TUI_UPDATE_TAG=""
	if _tui_update.release; then
		v="$(sed -n 's/.*"tag_name"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$tmp" | head -n1)"
		rm -f "$tmp"
		[[ -n "$v" ]] || {
			TUI_UPDATE_ERROR="no release found (use --dev for the main branch)"
			return 2
		}
		TUI_UPDATE_TAG="$v"
	else
	...
	v="${v//[[:space:]]/}"
	[[ "$v" =~ ^v?[0-9]+(\.[0-9]+)*([-+][0-9A-Za-z.]+)?$ ]] || {
		TUI_UPDATE_ERROR="the answer was not a version number"
		return 2
	}
...
tui.update.download() {
	...
	command -v tar >/dev/null 2>&1 || {
		TUI_UPDATE_ERROR="tar is not installed"
		return 1
	}
	mkdir -p "$dir/src" || {
		TUI_UPDATE_ERROR="cannot create $dir"
		return 1
	}
	archive="$dir/release.tar.gz"
	_tui_update.fetch "$(_tui_update.archive_url)" "$archive" || {
		[[ -n "$TUI_UPDATE_ERROR" ]] || TUI_UPDATE_ERROR="download failed"
		return 1
	}
	tar -xzf "$archive" -C "$dir/src" --strip-components=1 2>/dev/null || {
		TUI_UPDATE_ERROR="the download is not a valid archive"
		return 1
	}
	tui.sync.valid_source "$dir/src" || {
		TUI_UPDATE_ERROR="the download is not a DABT release"
		return 1
	}
...
tui.update.local() {
	...
	if [[ -d "$path" ]]; then
		...
		ln -s "$(cd -P "$path" && pwd -P)" "$dir/src" || {
			TUI_UPDATE_ERROR="cannot use $path"
			return 1
		}
	elif [[ "$path" == *.dapk ]]; then
		[[ -f "$path" ]] || {
			TUI_UPDATE_ERROR="not found: $path"
			return 1
		}
		...
		if ! dapk.verify.run "$path"; then
			TUI_UPDATE_ERROR="package verification failed: $DAPK_VERIFY_ERROR"
			dapk.verify.cleanup
			return 1
		fi
		...
		ln -s "$DAPK_VERIFY_DIR" "$dir/src" || {
			TUI_UPDATE_ERROR="cannot use $DAPK_VERIFY_DIR"
			return 1
		}
	elif [[ -f "$path" ]]; then
		...
		mkdir -p "$dir/src" || {
			TUI_UPDATE_ERROR="cannot create $dir/src"
			return 1
		}
		tar -xzf "$path" -C "$dir/src" --strip-components=1 2>/dev/null || {
			TUI_UPDATE_ERROR="$path is not a valid DABT release archive"
			return 1
		}
	else
		TUI_UPDATE_ERROR="not found: $path"
	...
	tui.sync.valid_source "$dir/src" || {
		TUI_UPDATE_ERROR="$path is not a DABT release (need VERSION, lib/tui.sh, share/defaults)"
		return 1
	}
	TUI_UPDATE_SCAN_HIGH=0 TUI_UPDATE_SCAN_WARN=0 TUI_UPDATE_SCAN_REPORT=""
...
tui.update.plan() {
	local src="$1"
	TUI_UPDATE_GIT=0
	[[ -e "$TUI_ROOT/.git" ]] && TUI_UPDATE_GIT=1
	tui.sync.plan "$src" "$TUI_HOME"
	if ((TUI_UPDATE_GIT)); then
		TUI_SYNC_P_ADD=()
		TUI_SYNC_P_UPDATE=()
		TUI_SYNC_P_SAME=()
		TUI_SYNC_P_REMOVE=()
...
tui.update.apply() {
	...
	tui.sync.valid_source "$src" || {
		TUI_UPDATE_ERROR="not a DABT release"
		return 1
	}
	read -r ver <"$src/VERSION"
	ver="${ver//[[:space:]]/}"
	printf -v stamp '%(%Y%m%d-%H%M%S)T' -1
	TUI_SYNC_STAMP="${TUI_SYNC_STAMP:-$stamp}"
	tui.sync.apply "$src" "$TUI_HOME" "$resolver" || {
		TUI_UPDATE_ERROR="config sync failed"
		return 1
	}
	...
	if [[ -e "$TUI_ROOT/.git" ]]; then
		TUI_UPDATE_RESULT+=("program: $TUI_ROOT is a git checkout - run  git pull  there for the code")
	else
		tui.sync.program_apply "$src" "$TUI_ROOT" "$TUI_HOME/backups/$TUI_SYNC_STAMP" || {
			TUI_UPDATE_ERROR="program copy failed"
			return 1
		}
	...
	} >"$TUI_HOME/install.meta.tmp" && mv -f "$TUI_HOME/install.meta.tmp" "$TUI_HOME/install.meta"
...
_tui_update.cli_resolver() { # REL -> asks on the terminal (or uses the policy with --yes)
	local rel="$1" a
	if [[ -n "${_TUI_UPDATE_CLI_POLICY:-}" ]]; then
		printf '%s' "$_TUI_UPDATE_CLI_POLICY"
		return
	fi
	while true; do
...
tui.update.cli() {
	...
	while (($#)); do
		case "$1" in
			--check) check=1 ;; --path)
				...
				;;
			--trust-key)
				TUI_UPDATE_TRUST_KEY="$2"
				shift
				;;
			--allow-unsigned) TUI_UPDATE_ALLOW_UNSIGNED=1 ;;
			--dev) TUI_UPDATE_CHANNEL=dev ;; --release) TUI_UPDATE_CHANNEL=release ;; --yes | -y) yes=1 ;; --force | -f) force=1 ;; --no-scan) TUI_UPDATE_NOSCAN=1 ;; --policy)
				policy="$2"
				shift
				;;
		esac
	...
	if [[ -n "$path" ]]; then
		...
		tui.update.local "$path" "$tmp" || {
			printf 'dabt: not using %s: %s\n' "$path" "$TUI_UPDATE_ERROR" >&2
			return 1
		}
		...
		read -r TUI_UPDATE_LATEST <"$tmp/src/VERSION"
		TUI_UPDATE_LATEST="${TUI_UPDATE_LATEST//[[:space:]]/}"
		if tui.version.newer "$TUI_UPDATE_LATEST" "$TUI_VERSION"; then
			printf 'Version %s is newer than what you have (%s).\n' "$TUI_UPDATE_LATEST" "$TUI_VERSION"
		...
		if ((check)); then
			_tui_update.news_since_file "$tmp/src/CHANGELOG.md" "$TUI_VERSION"
			return 10
		fi
	else
		...
		case $rc in
			2)
				...
				;;
			1)
				printf 'You are up to date (latest: %s).\n' "${TUI_UPDATE_LATEST:-$TUI_VERSION}"
				return 0
				;;
		esac
		printf 'A newer version is available: %s (you have %s).\n' "$TUI_UPDATE_LATEST" "$TUI_VERSION"
		if ((check)); then
			_tui_update.news_since "$TUI_VERSION"
			return 10
		fi
		tui.update.download "$tmp" || {
			printf 'Download failed: %s\n' "$TUI_UPDATE_ERROR" >&2
			return 1
		}
	fi
	if [[ "${TUI_UPDATE_NOSCAN:-0}" != 1 ]]; then
		printf "Security scan of the %s:\n" "$([[ -n "$path" ]] && echo copy || echo download)"
		...
		if ((TUI_UPDATE_SCAN_HIGH && ! force)); then
			printf "Update refused: %d high-risk finding(s). Review them, then rerun with --force to apply anyway.\n" "$TUI_UPDATE_SCAN_HIGH" >&2
			return 3
		fi
	fi
	...
	_TUI_UPDATE_CLI_SRC="$tmp/src"
	_TUI_UPDATE_CLI_POLICY=""
	if ((yes)); then _TUI_UPDATE_CLI_POLICY="${policy:-new}"; elif [[ -n "$policy" ]]; then _TUI_UPDATE_CLI_POLICY="$policy"; fi
	tui.update.apply "$tmp/src" _tui_update.cli_resolver || {
...
_tui_sync.resolve_next() {
	...
	while ((_RUI_I < ${#TUI_SYNC_CONFLICT[@]})); do
		...
		if [[ -n "$_RUI_ALL" ]]; then
			TUI_SYNC_DECISION[$rel]="$_RUI_ALL"
			((_RUI_I++))
			continue
		fi
...
_tui_sync.resolve_pick() {
	...
	case "$1" in
		...
		3)
			local d
			d="$(diff -u "$TUI_HOME/$rel" "$(_tui_sync.config_src "$_RUI_SRC" "$rel")" 2>&1)"
			[[ -n "$d" ]] || d="(the files are identical)"
			tui.view "Differences: $rel   (- yours, + new)" "$d" --cancel _tui_sync.resolve_next
			return 0
			;;
		4)
			_RUI_ALL=override
			TUI_SYNC_DECISION[$rel]=override
			;;
		5)
			_RUI_ALL=skip
			TUI_SYNC_DECISION[$rel]=skip
			;;
		6)
			_RUI_ALL=new
			TUI_SYNC_DECISION[$rel]=new
			;;
	esac
	((_RUI_I++))
	_tui_sync.resolve_next
}
_tui_sync.resolve_cancel() {
	tui.notify "Update cancelled: nothing was changed" info 4
	_tui_update.cleanup
}

declare -g _TUI_UPDATE_TMP="" _TUI_UPDATE_SRC=""
_tui_update.cleanup() {
	[[ -n "$_TUI_UPDATE_TMP" && -d "$_TUI_UPDATE_TMP" ]] && rm -rf "$_TUI_UPDATE_TMP"
	_TUI_UPDATE_TMP=""
}
...
tui.action.update() {
	...
	_tui_update.cleanup
	_TUI_UPDATE_TMP="$(mktemp -d)"
	if ! tui.update.download "$_TUI_UPDATE_TMP"; then
		tui.notify "Update failed: $TUI_UPDATE_ERROR" error 8
		_tui_update.cleanup
		return 1
	fi
	...
	local newv
	read -r newv <"$_TUI_UPDATE_SRC/VERSION"
	local rep
	rep="$(tui.sync.report)"
...
_tui_update.ui_reviewed() {
	if ((${#TUI_SYNC_CONFLICT[@]})); then
		tui.sync.resolve_ui _tui_update.ui_confirm "$_TUI_UPDATE_SRC"
