...
_ts_detect() {
	...
	if [[ -n "${KITTY_WINDOW_ID:-}" || "$TERM" == xterm-kitty ]]; then
		...
		TS_TERM=ghostty
	elif [[ -n "${ALACRITTY_SOCKET:-}${ALACRITTY_LOG:-}" || "$TERM" == alacritty* ]]; then
		TS_TERM=alacritty
	elif [[ "${TERM_PROGRAM:-}" == iTerm.app ]]; then
		TS_TERM=iterm2
	elif [[ "${TERM_PROGRAM:-}" == Apple_Terminal ]]; then
		TS_TERM=apple-terminal
	elif [[ -n "${WT_SESSION:-}" ]]; then
		TS_TERM=windows-terminal
	elif [[ -n "${KONSOLE_VERSION:-}" ]]; then
		TS_TERM=konsole
		TS_VER="$KONSOLE_VERSION"
	elif [[ "$TERM" == foot* ]]; then
		TS_TERM=foot
	elif [[ -n "${GNOME_TERMINAL_SCREEN:-}${GNOME_TERMINAL_SERVICE:-}" ]]; then
		TS_TERM=gnome-terminal
		TS_VER="${VTE_VERSION:-}"
		...
		TS_TERM=xterm
		TS_VER="$XTERM_VERSION"
	elif [[ -n "${TERM_PROGRAM:-}" ]]; then
		TS_TERM="${TERM_PROGRAM,,}"
	else _ts_xtversion; fi
	[[ -n "$TS_TERM" ]] || TS_TERM=unknown
	# what can be freed at runtime
	if [[ "$TS_TERM" == kitty ]] && command -v kitty >/dev/null && [[ -n "${KITTY_PID:-}" ]]; then
		TS_STRATEGY=kitty
	elif [[ "$TS_TERM" == gnome-terminal || "$TS_TERM" == vte ]] && _ts_gnome_ok; then
		TS_STRATEGY=gnome
	elif [[ "$TS_MUX" == tmux ]] && command -v tmux >/dev/null; then
		TS_STRATEGY=tmux
	fi
}
...
_ts_xtversion() {
	...
	local c buf=""
	printf '\e[>0q'
	while IFS= read -rsn1 -t 0.15 c; do
		buf+="$c"
		[[ "$buf" == *$'\e\\' || ${#buf} -gt 120 ]] && break
	done
	[[ "$buf" == *'>|'* ]] || return 0
	buf="${buf#*>|}"
	buf="${buf%%$'\e'*}"
...
_ts_collect_used() {
	...
	for k in "${!_TUI_BIND[@]}"; do
		c="${k#*|}"
		_ts_note_chord "$c" "bind: ${_TUI_BIND[$k]}"
	done
	for k in "${!_TUI_UBIND[@]}"; do
		c="${k#*|}"
		_ts_note_chord "$c" "your bind: ${_TUI_UBIND[$k]}"
	done
...
_ts_kitty_free() { # QUIET
	_ts_kitty_dir
	local conf="$_KD/kitty.conf" inc="$_KD/dabt-shortcuts.conf" c
	[[ -f "$conf" ]] || {
		_ts_msg warn "kitty.conf not found in $_KD"
		return 1
	}
	...
	_ts_kitty_conflicts
	((${#TS_CONF[@]})) || {
		[[ "$1" == quiet ]] || _ts_msg success "Nothing to free: kitty keeps none of the keys DABT uses"
		return 0
	}
	local n=${#TS_CONF[@]}
	{
	...
	kill -SIGUSR1 "$KITTY_PID" 2>/dev/null || {
		_ts_msg error "could not signal kitty (KITTY_PID=$KITTY_PID)"
		return 1
	}
...
_ts_kitty_restore() {
	_ts_kitty_dir
	local inc="$_KD/dabt-shortcuts.conf"
	[[ -f "$inc" ]] || return 0
...
_ts_tmux_conflicts() {
	...
	while IFS= read -r line; do
		...
		_ts_tmux_chord "$k"
		[[ -n "${_TS_USED[$_CH]:-}" ]] || continue
		TS_CONF+=("$_CH"$'\t'"tmux root binding: $k   (DABT: ${_TS_USED[$_CH]})")
		_TS_TKEY[$_CH]="$k"
...
_ts_tmux_restore() {
	...
	[[ -f "$bk" ]] || return 0
	tmux source-file "$bk" 2>/dev/null
	rm -f "$bk"
}
...
_ts_free() { # [quiet]
	...
	case "$TS_STRATEGY" in
		...
		*) [[ "$1" == quiet ]] || {
			_ts_msg warn "Cannot free shortcuts at runtime in ${TS_TERM}${TS_MUX:+ (in $TS_MUX)}: showing the config to add"
			_ts_cmd_snippet
		} ;;
	esac
}
...
_ts_on_ready() {
	[[ "$(tui.plugin.config terminal_shortcuts auto_free)" == 1 ]] && _ts_free quiet
	return 1
}
_ts_on_exit() {
	[[ -n "$_TS_FREED" ]] && _ts_restore quiet
	return 1
}
...
_ts_report() {
	_ts_detect
	local n
	n="$(tui.plugin.config terminal_shortcuts blocked)"
	printf 'Terminal      %s %s\n' "$TS_TERM" "$TS_VER"
...
_ts_cmd_conflicts() {
	...
	case "$TS_STRATEGY" in
		...
		*)
			tui.view "Shortcuts the terminal takes" "Live listing is not available for ${TS_TERM}."$'\n\n'"Use \"Terminal: check shortcuts\" (works everywhere): press the keys and see which never arrive."
			return
			;;
	esac
	local out="" row
	if ((${#TS_CONF[@]})); then
		for row in "${TS_CONF[@]}"; do
			printf -v row '%-24s %s' "${row%%$'\t'*}" "${row#*$'\t'}"
			out+="$row"$'\n'
		done
...
_ts_probe_next() {
	((_TSP_I++))
	if ((_TSP_I >= ${#_TSP_LIST[@]})); then _ts_probe_done; else
		tui.modal.redraw
		_ts_probe_arm
	fi
}
...
_ts_probe_key() {
	...
	_TSP_LAST="$k"
	tui.modal.redraw # something else arrived: keep waiting
}
_ts_probe_draw() {
	tui.modal.active ts_probe || return 0
...
_ts_snippet_chords() { # -> _SC: the chords to free (the last check's result, else the usual suspects)
	local b
	b="$(tui.plugin.config terminal_shortcuts blocked)"
...
_ts_cmd_snippet() {
	...
	case "$TS_TERM" in
		kitty)
			...
			out="${out//pgdn/page_down}"
			;;
		alacritty)
			out+="# alacritty.toml   (ReceiveChar sends the key to the program)"$'\n[keyboard]\nbindings = [\n'
			for c in $_SC; do
				_ts_snippet_split "$c"
				_ts_snippet_gtkname "$_Kx"
				m=""
				for mods in "${_M[@]}"; do m+="${m:+|}${mods^}"; done
				m="${m/Ctrl/Control}"
		...
		windows-terminal)
			...
			;;
		gnome-terminal | vte)
			out+="# gsettings works at runtime for GNOME Terminal - run \"Terminal: free shortcuts\"."$'\n'"# Other VTE terminals: Preferences > Shortcuts, set these to Disabled:"$'\n'
			for c in $_SC; do out+="  $c"$'\n'; done
			;;
		konsole)
			out+="Konsole: Settings > Configure Keyboard Shortcuts, remove these:"$'\n'
			for c in $_SC; do out+="  $c"$'\n'; done
			;;
		foot)
			out+="foot.ini [key-bindings] / [text-bindings]: set the action that uses each of these to 'none':"$'\n'
			for c in $_SC; do out+="  $c"$'\n'; done
			;;
		iterm2)
			out+="iTerm2: Settings > Keys > Key Bindings: remove or override these:"$'\n'
			for c in $_SC; do out+="  $c"$'\n'; done
			;;
		*)
