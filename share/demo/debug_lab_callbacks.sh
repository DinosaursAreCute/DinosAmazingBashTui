...
lab_dlg_confirm() { _lab_log "tui.confirm"; tui.confirm "Log a line to the panel on the right?" lab_dlg_yes lab_dlg_no --title "Log it?"; }
lab_dlg_danger()  { _lab_log "tui.confirm --danger"; tui.confirm "Wipe the log? This cannot be undone." lab_dlg_wipe lab_dlg_no --danger --yes Wipe --no Keep; }
lab_dlg_wipe()    { _LAB_LOG=(); tui.notify "Log wiped" warn 2; _lab_log "log wiped"; }
...
lab_dlg_digits()  { [[ "$1" =~ ^[0-9]+$ ]] || { TUI_DIALOG_ERROR="numbers only, please"; return 1; }; }
lab_dlg_port()    { _lab_log "prompt submitted: '$1'"; tui.notify "Port set to $1" success; }
...
lab_dlg_cancelled() { _lab_log "prompt cancelled"; tui.notify "Cancelled" info 2; }
lab_dlg_picked()  { _lab_log "chose #$1: $2"; local f="${TUI_THEMES_DIR:-}/$2.css"; if [[ "$2" == default ]]; then tui.theme.clear; elif [[ -r "$f" ]]; then tui.theme.set "$f"; fi; tui.notify "Theme: $2" success 2; }
lab_dlg_choose() {
    local -a names=(default); local f
    for f in "${TUI_THEMES_DIR:-/nonexistent}"/*.css; do [[ -e "$f" ]] && { f="${f##*/}"; names+=("${f%.css}"); }; done
...
lab_dlg_deleted() { _lab_log "deleted '$1' (confirm inside a prompt callback)"; tui.notify "Deleted $1" success; }
...
lab_toast_clear()   { tui.notify.clear; _lab_log "tui.notify.clear"; }
lab_toast_page()    { tui.notify "This toast follows you to the next page" success 6; tui.action.goto debug.xml; }
