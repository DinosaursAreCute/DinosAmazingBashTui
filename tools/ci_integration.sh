...
work="$(mktemp -d "${TMPDIR:-/tmp}/dabt_it.XXXXXX")"; trap 'rm -rf "$work"' EXIT
step() { printf '\n== %s\n' "$*"; }
...
env_for() { # NAME -> exports HOME etc. for that install
    ...
    export PATH="$HOME/.local/bin:$ORIG_PATH"; mkdir -p "$HOME"
...
mkdir -p "$work/pkg"; tar -xzf "$pkg" -C "$work/pkg" --strip-components=1
[[ -f "$work/pkg/install.sh" && -f "$work/pkg/VERSION" ]] || fail "$pkg does not contain install.sh and VERSION"
read -r want < "$work/pkg/VERSION"; want="${want//[[:space:]]/}"

step "1/3 fresh install from ${pkg##*/}"
env_for fresh
bash "$work/pkg/install.sh" --yes --no-scan
got="$(dabt --version)"; [[ "$got" == "DABT $want" ]] || fail "dabt --version says '$got', expected 'DABT $want'"
