...
_plg_human() {
	local b=$1
	if ((b >= 1048576)); then _H="$((b / 1048576)).$((b % 1048576 * 10 / 1048576)) MB"; elif ((b >= 1024)); then _H="$((b / 1024)).$((b % 1024 * 10 / 1024)) KB"; else _H="$b B"; fi
}
...
_plg_name() {
	local r
	r="$(tui.table.row plg_table)"
	printf '%s' "${r%%|*}"
}

plg_visit() {
	tui.hook.on resize plg_rewrap
	_PLG_CLOSED=()
	_plg_fill 0
	plg_selected
	tui.focus plg_table
...
_plg_details() {
	...
	title="$(tui.plugin.get "$n" title)"
	ver="$(tui.plugin.get "$n" version)"
	auth="$(tui.plugin.get "$n" author)"
	req="$(tui.plugin.get "$n" requires)"
	desc="$(tui.plugin.get "$n" description)"
	state="$(tui.plugin.get "$n" state)"
	src="$(tui.plugin.get "$n" source)"
	file="$(tui.plugin.get "$n" file)"
	root="$(tui.plugin.root "$n")"
	err="$(tui.plugin.get "$n" error)"
	_plg_human "${st[bytes]:-0}"
	tui.pane_size plg_details
	export TR_WIDTH=$((TUI_PANE_COLS > 12 ? TUI_PANE_COLS - 1 : 12))
	local out="" blk
	out+="$title  v$ver"$'\n'
	_plg_wrap "$desc" "$TR_WIDTH" 0
	out+="$(printf '%s\n' "${_WRAP[@]}")"$'\n'
	...
	if ((${#cfg[@]})); then
		blk="$(kv_string "${cfg[@]}" -t dots)"
		_plg_nl "$(divider_string 'Saved settings')\n$blk"
		out+="$_NL"$'\n'
	fi
	local -a cmds=()
	for id in "${!_TUI_CMD_PLUGIN[@]}"; do [[ "${_TUI_CMD_PLUGIN[$id]}" == "$n" ]] && cmds+=("${_TUI_CMD_TITLE[$id]}: $id"); done
	if ((${#cmds[@]})); then
		blk="$(kv_string "${cmds[@]}" -t dots)"
		_plg_nl "$(divider_string 'Command bar')\n$blk"
		out+="$_NL"$'\n'
	fi
	unset TR_WIDTH
...
_plg_walk() { # DIR DEPTH
	...
	for f in "$d"/*; do
		...
		_PLG_PATH+=("$f")
		_PLG_DIR+=(1)
		if [[ -n "${_PLG_CLOSED[$f]:-}" ]]; then
			_PLG_LABELS+=("$pad▸ $name/")
		else
			_PLG_LABELS+=("$pad▾ $name/")
			_plg_walk "$f" $((depth + 1))
		fi
...
plg_selected() {
	...
	local i f main
	main="$(tui.plugin.get "$n" file)"
	for i in "${!_PLG_PATH[@]}"; do [[ "${_PLG_PATH[i]}" == "$main" ]] && {
		tui.list.select plg_files "$i"
		break
	}; done
	_plg_show "$main"
}
...
declare -g _PLG_MENU=""
plg_enter() {
	local n st desc info toggle
	n="$(_plg_name)"
	[[ -n "$n" ]] || return 0
	_PLG_MENU="$n"
	st="$(tui.plugin.get "$n" state)"
	desc="$(tui.plugin.get "$n" description)"
	local -A stt=()
	local k v
...
plg_rescan() {
	tui.plugin.scan
	_PLG_NAME=""
	_plg_fill "$(tui.table.selected plg_table)"
	plg_selected
	tui.notify "Scanned: $(tui.plugin.list | wc -l) plugins" info 2
}
...
_plg_show() {
	...
	w=${#lines[@]}
	gw=${#w}
	tui.pane_size plg_content
	local avail=$((TUI_PANE_COLS - 1)) # one column for the scroll indicator
	for line in "${lines[@]}"; do
		...
		for i in "${!_WRAP[@]}"; do
			if ((i == 0)); then
				printf -v line '%*d  %s' "$gw" "$n" "${_WRAP[i]}"
...
plg_rewrap() {
	[[ -n "$(tui.get.type plg_content 2>/dev/null)" && -n "$_PLG_SHOWN" ]] && _plg_show "$_PLG_SHOWN"
	return 1
}
plg_file_selected() {
	local i
	i="$(tui.list.selected plg_files)"
	((i >= 0)) || return 0
...
plg_toggle() {
	local n sel
	n="$(_plg_name)"
	sel="$(tui.table.selected plg_table)"
	[[ -n "$n" ]] || return 0
	if tui.plugin.toggle "$n"; then
		tui.notify "$n: $(tui.plugin.get "$n" state)" info 2
...
plg_reload() {
	...
	[[ -n "$n" ]] || return 0
	tui.plugin.reload "$n"
	tui.notify "$n reloaded" success 2
	_PLG_NAME=""
	_plg_fill "$sel"
	plg_selected
}
...
plg_do_install() {
	local p="${1/#\~/$HOME}"
	if tui.plugin.install "$p"; then
		tui.notify "Installed $TUI_PLUGIN_NAME (not enabled yet)" success 4
		_PLG_NAME=""
		_plg_fill 0
		plg_selected
...
plg_scan() {
	...
	root="$(tui.plugin.root "$n")"
	[[ -e "$root" ]] || {
		tui.notify "$n: nothing to scan (no files)" error 4
		return 0
	}
	tui.notify "Scanning $n ..." info 1
	tui.scan.run "$root"
	if ((TUI_SCAN_HIGH)); then
		verdict="HIGH RISK: $TUI_SCAN_HIGH high-risk finding(s), $TUI_SCAN_WARN warning(s)"
	elif ((TUI_SCAN_WARN)); then
		verdict="No high-risk findings, $TUI_SCAN_WARN warning(s)"
