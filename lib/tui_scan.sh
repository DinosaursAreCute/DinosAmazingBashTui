...
_scan.files() {   # PATH -> script files, one per line
    ...
    while IFS= read -r f; do
        ...
        IFS= read -r first < "$f" 2>/dev/null; [[ "$first" == '#!'*sh* ]] && printf '%s\n' "$f"
...
tui.scan.run() {
    ...
    [[ -e "$path" ]] || { TUI_SCAN_NOTES="scan: not found: $path"; return 1; }
    mapfile -t files < <(_scan.files "$path")
    (( ${#files[@]} )) || { TUI_SCAN_NOTES="no shell scripts found"; return 0; }
    for f in "${files[@]}"; do
    ...
    if [[ "$deep" == 1 ]]; then
        if command -v semgrep >/dev/null 2>&1; then
            n=0
            while IFS= read -r hit; do
                [[ -n "$hit" ]] || continue; ((++n)); ((++TUI_SCAN_WARN)); out+="  WARN $hit  (semgrep)"$'\n'
...
tui.scan.tools() {
    ...
    for t in shellcheck semgrep; do
        if command -v "$t" >/dev/null 2>&1; then printf '%-11s installed (%s)\n' "$t" "$(command -v "$t")"
        else printf '%-11s not installed   dabt scan --install %s\n' "$t" "$t"; fi
    done
    printf 'package manager: %s\n' "$(_scan.pm || echo 'none found')"
}

_scan.pm() { local p; for p in apt-get dnf pacman zypper apk brew nix-env pipx pip3 pip; do command -v "$p" >/dev/null 2>&1 && { echo "$p"; return 0; }; done; return 1; }
...
_scan.install_cmd() {
    ...
    case "$tool" in
        shellcheck)
            for pm in apt-get dnf pacman zypper apk brew nix-env; do command -v "$pm" >/dev/null 2>&1 || continue
                case "$pm" in
        ...
        semgrep)
            for pm in pipx brew pip3 pip; do command -v "$pm" >/dev/null 2>&1 || continue
                case "$pm" in
...
tui.scan.install() {
    ...
    [[ "${2:-}" == --yes || "${2:-}" == -y ]] && yes=1
    case "$tool" in shellcheck|semgrep) ;; *) echo "dabt scan: unknown tool '$tool' (shellcheck | semgrep)" >&2; return 2 ;; esac
    if command -v "$tool" >/dev/null 2>&1; then echo "$tool is already installed"; return 0; fi
    cmd="$(_scan.install_cmd "$tool")" || { echo "dabt scan: no known package manager found for $tool; install it manually" >&2; return 1; }
    echo "Will run: $cmd"
    if (( ! yes )); then read -r -p "Install $tool? [y/N] " a; [[ "$a" == [yY]* ]] || { echo aborted; return 1; }; fi
    bash -c "$cmd" || {
...
tui.scan.cli() {
    ...
    while (( $# )); do
        a="$1"; shift
        case "$a" in
            --tools|tools) tui.scan.tools; return ;;
            --install)     tui.scan.install "${1:-}" "${2:-}"; return ;;
            --deep)        deep=1 ;; --strict) strict=1 ;;
            -h|--help|help|"") ;;
            -*)            echo "dabt scan: unknown option $a" >&2; return 2 ;;
            *)             paths+=("$a") ;;
        esac
    done
    if (( ! ${#paths[@]} )); then cat <<'HELP'
