...
tui.every.clear() {
	local id
	for id in "${_TA_IDS[@]}"; do _tui_api._job_del "$id"; done
}
tui.every.list() {
	local id
	for id in "${_TA_IDS[@]}"; do printf '%s\t%sms\t%s\n' "$id" "${_TA_INT[$id]}" "${_TA_FN[$id]}"; done
}
...
_tui_api._clock_run() {
	...
	if [[ -n "$font" ]]; then
		tui.style.sgr "$pane"
		local ansi="$TUI_SGR" # fork-free (tui.ansi was a subshell per tick)
...
tui.watch() {
	...
	[[ -z "$pane" || -z "$cmd" ]] && {
		echo "tui.watch: need PANE and CMD" >&2
		return 1
	}
	tui.watch.stop "$id"
	_tui_api._tmp_init
	...
	local s
	printf -v s "%d.%03d" $((_TA_MS / 1000)) $((_TA_MS % 1000))
	(
...
tui.watch.stop() {
	...
	kill "${_TA_WPID[$id]}" 2>/dev/null
	wait "${_TA_WPID[$id]}" 2>/dev/null
	local cfd=${_TA_WFD[$id]}
	exec {cfd}<&-
	rm -f "$_TA_TMP/$id"
	...
	for id in "${_TA_WIDS[@]}"; do
		...
		while read -t 0 -u "$fd"; do
			if IFS= read -r -d $'\x1e' -t 0.02 -u "$fd" f; then
				frame="${_TA_WBUF[$id]}$f"
				_TA_WBUF[$id]=""
				got=1 # keep newest complete frame only
			else
				_TA_WBUF[$id]+="$f"
				break # partial: finish next tick
			fi
		done
		((got)) || continue
		if [[ -z "${_TUI_P_ROW[${_TA_WPANE[$id]}]:-}" ]]; then
			tui.watch.stop "$id"
			continue
		fi
...
tui.sys.mem() {
	...
	TUI_SYS_MEM_USED_MB=$(((total - avail) / 1024))
	TUI_SYS_MEM_TOTAL_MB=$((total / 1024))
...
_tui_api._monitor_run() {
	...
	local out="" line
	_gauge_build -w "$gw" -lw 5 -l "CPU" "$TUI_SYS_CPU"
	printf -v line '%b' "${TR_RESULT[0]}"
	out+="$line"$'\n'
	_gauge_build -w "$gw" -lw 5 -l "MEM" "$TUI_SYS_MEM_PCT"
	printf -v line '%b' "${TR_RESULT[0]}"
	out+="$line"$'\n'
	_gauge_build -w "$gw" -lw 5 -l "SWAP" "$TUI_SYS_SWAP_PCT"
	printf -v line '%b' "${TR_RESULT[0]}"
	out+="$line"$'\n\n'
...
tui.get.style() {
	...
	case "$field" in
		...
		mods) printf '%s\n' "${_TUI_STYLE_MOD[$key]:-}" ;;
		*)
			echo "tui.get.style: FIELD must be fg|bg|mods" >&2
			return 2
			;;
	esac
}
...
tui.get.class.style() {
	...
	case "$field" in
		...
		mods) printf '%s\n' "${_TUI_CLASS_MOD[$key]:-}" ;;
		*)
			echo "tui.get.class.style: FIELD must be fg|bg|mods" >&2
			return 2
			;;
	esac
}
...
tui.class.style() {
	...
	[[ "$state" != normal ]] && key="${cls}_${state}"
	if [[ -z "${_TUI_CLASS_FG[$key]:-}${_TUI_CLASS_BG[$key]:-}${_TUI_CLASS_MOD[$key]:-}" ]]; then key="$cls"; fi
	TUI_FG="${_TUI_CLASS_FG[$key]:-}"
	TUI_BG="${_TUI_CLASS_BG[$key]:-}"
	TUI_MODS="${_TUI_CLASS_MOD[$key]:-}"
}
tui.class.sgr() {
	tui.class.style "$@"
...
tui.style.sgr() {
	...
	TUI_FG="${_TUI_STYLE_FG[$key]:-}"
	TUI_BG="${_TUI_STYLE_BG[$key]:-}"
	_tui._style_v "$key"
	TUI_SGR="$_SGR"
}
