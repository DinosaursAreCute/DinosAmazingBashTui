...
[[ -r "$TUI_ROOT/VERSION" ]] && { read -r TUI_VERSION < "$TUI_ROOT/VERSION"; TUI_VERSION="${TUI_VERSION//[[:space:]]/}"; }
...
_tui_home.locate() {
    ...
    if [[ -n "${TUI_HOME:-}" ]]; then TUI_HOME_SOURCE=TUI_HOME
    elif [[ -d "$xdg/DABT" ]]; then TUI_HOME="$xdg/DABT"; TUI_HOME_SOURCE=default
    elif [[ -n "${DABT_HOME:-}" && -d "$DABT_HOME" ]]; then TUI_HOME="$DABT_HOME"; TUI_HOME_SOURCE=DABT_HOME
    else
        TUI_HOME=""
        ...
        [[ -n "$TUI_HOME" ]] || { TUI_HOME="$xdg/DABT"; TUI_HOME_SOURCE=missing; }
    fi
}
_tui_home.locate
declare -g TUI_HOME
declare -g TUI_INSTALLED=0; [[ -f "$TUI_HOME/install.meta" ]] && TUI_INSTALLED=1
...
tui.app.meta_get() { [[ -n "${_TUI_META[$1]+x}" ]] || _tui_home.meta_load; printf '%s' "${_TUI_META[$1]-${2:-}}"; }
tui.app.meta_set() { _tui_home.meta_load; _TUI_META[$1]="$2"; _tui_home.meta_save; }
