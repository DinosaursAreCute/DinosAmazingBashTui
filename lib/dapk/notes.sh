...
_dapk.notes.value() { # VAR -> _NV ; rc 1 for an unknown variable
	...
	if [[ "$v" == section:* ]]; then
		_NV=""
		[[ -n "$DAPK_NOTES_CHANGELOG" && -r "$DAPK_NOTES_CHANGELOG" ]] && _NV="$(dapk.changelog.section "$DAPK_NOTES_CHANGELOG" "$DAPK_NOTES_VERSION" "${v#section:}")"
		return 0
	fi
...
dapk.notes.render() {
	...
	DAPK_NOTES_ERROR=""
	[[ -r "$tpl" ]] || {
		DAPK_NOTES_ERROR="release template not readable: $tpl"
		return 1
	}
	: >"$out"
	while IFS= read -r line || [[ -n "$line" ]]; do
		...
		if [[ "$t" =~ ^\{\{#if[[:space:]]+([^}[:space:]]+)[[:space:]]*\}\}$ ]]; then
			_dapk.notes.value "${BASH_REMATCH[1]}" || {
				DAPK_NOTES_ERROR="line $n: unknown variable '${BASH_REMATCH[1]}'"
				return 1
			}
			[[ -n "${_NV//[[:space:]]/}" ]] && stack+=(1) || stack+=(0)
			continue
		fi
		if [[ "$t" == "{{/if}}" ]]; then
			((${#stack[@]})) || {
				DAPK_NOTES_ERROR="line $n: {{/if}} without {{#if}}"
				return 1
			}
			unset 'stack[${#stack[@]}-1]'
			continue
		...
		while [[ "$rest" =~ \{\{([^{}]+)\}\} ]]; do
			...
			v="${v%"${v##*[![:space:]]}"}"
			_dapk.notes.value "$v" || {
				DAPK_NOTES_ERROR="line $n: unknown variable '$v'"
				return 1
			}
			res+="${rest%%"${BASH_REMATCH[0]}"*}$_NV"
			rest="${rest#*"${BASH_REMATCH[0]}"}"
		done
		printf '%s\n' "$res$rest" >>"$out"
	done <"$tpl"
	((${#stack[@]} == 0)) || {
		DAPK_NOTES_ERROR="unclosed {{#if}}"
		return 1
	}
...
dapk.notes.compare_url() {
	...
	while IFS= read -r t; do
		...
		dapk.version.cmp "${t#v}" "${tag#v}"
		((DAPK_VCMP < 0)) || continue
		if [[ -z "$best" ]]; then best="$t"; else
			dapk.version.cmp "${t#v}" "${best#v}"
			((DAPK_VCMP > 0)) && best="$t"
		fi
