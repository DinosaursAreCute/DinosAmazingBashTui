...
_dapk.install.unzip() {
    local zip="$1" dir="$2" f; local -a found=()
    mkdir -p "$dir" || return 1
    if command -v unzip >/dev/null 2>&1; then unzip -qq -o "$zip" -d "$dir" >/dev/null 2>&1
    elif command -v bsdtar >/dev/null 2>&1; then bsdtar -xf "$zip" -C "$dir" >/dev/null 2>&1
    else dapk.ui.err "unzip (or bsdtar) is required to open ${zip##*/}"; return 1; fi || { dapk.ui.err "cannot extract ${zip##*/}"; return 1; }
    while IFS= read -r f; do found+=("$f"); done < <(find "$dir" -type f -name "*.$DABT_PKG_EXT" -print | sort)
    (( ${#found[@]} == 1 )) || { dapk.ui.err "${zip##*/} must contain exactly one .$DABT_PKG_EXT (found ${#found[@]})"; return 1; }
...
dapk.install.run() {
    ...
    while (( $# )); do
        if _dapk.cmd.ui "$1" "${2:-}"; then [[ "$1" == --strict ]] && pass+=(--strict); shift "$_UI_SHIFT"; continue; fi
        case "$1" in
            --install-dependencies) DAPK_DEPS_MODE=install; install_dep=1; shift ;;
            --no-deps) DAPK_DEPS_MODE=none; no_deps=1; shift ;;
            --require-deps) DAPK_DEPS_REQUIRE=1; shift ;;
            --yes|-y) DAPK_DEPS_YES=1; shift ;;
            --allow-custom-install) DAPK_DEPS_ALLOW_CUSTOM=1; shift ;;
            --pm) DAPK_DEPS_PM="${2:-}"; shift 2 ;;
            --trust-key) DAPK_VERIFY_TRUST_KEY="${2:-}"; shift 2 ;;
            --allow-unsigned) DAPK_VERIFY_ALLOW_UNSIGNED=1; shift ;;
            --plan) plan=1; DAPK_DEPS_PLAN_ONLY=1; shift ;;
            --install-path) pass+=(--install-path "${2:-}"); shift 2 ;;
            --force|-f|--no-scan) pass+=("$1"); shift ;;
            --name|--entry) pass+=("$1" "${2:-}"); shift 2 ;;
            -h|--help) _dapk.cmd.help; return 0 ;;
            -*) _dapk.cmd.usage "install: unknown option $1"; return 2 ;;
            *) [[ -z "$src" ]] || { _dapk.cmd.usage "install: one package only"; return 2; }; src="$1"; shift ;;
        esac
    done
    [[ -n "$src" ]] || { _dapk.cmd.usage "install: give a .$DABT_PKG_EXT file or URL"; return 2; }
    if (( no_deps && ( install_dep || DAPK_DEPS_REQUIRE ) )); then _dapk.cmd.usage "--no-deps cannot be combined with --install-dependencies or --require-deps"; return 2; fi

    dapk.ui.init; dapk.ui.steps 4
    ...
    if [[ "$src" == http://* || "$src" == https://* ]]; then
        command -v curl >/dev/null 2>&1 || { dapk.ui.err "curl is required to download a package"; rc=1; }
        if (( rc == 0 )); then
            pkg="$work/${src##*/}"; pkg="${pkg%%\?*}"
    ...
    if (( rc == 0 )); then
        name="${DAPK_MANIFEST_H[name]}"; ver="${DAPK_MANIFEST_H[version]}"
        dapk.ui.step "Package"
        dapk.ui.kv name "$name"; dapk.ui.kv version "$ver-${DAPK_MANIFEST_H[build]:-0}"; dapk.ui.kv signer "${DAPK_VERIFY_FPR:-unsigned}"
        ...
        since=""; _tui_apps.lookup "$name" && since="$A_VERSION"
    ...
    if (( rc == 0 && plan )); then dapk.ui.step "Install"; dapk.ui.info "plan only: nothing was installed"; fi
    if (( rc == 0 && ! plan )); then
        dapk.ui.step "Installing"
        [[ "$src" == http://* || "$src" == https://* ]] && :
        tui.apps.install "$DAPK_VERIFY_DIR" --origin "$src" "${pass[@]}"; rc=$?
        if (( rc == 0 )); then (( DAPK_DEPS_UNMET )) && { dapk.deps.mark "$name" unmet; dapk.ui.warn "$name has unmet dependencies: run  dabt pkg deps $name"; } || dapk.deps.mark "$name" ok; fi
    fi
    rm -rf "$work"
    _dapk.cmd.finish "$rc"
}
