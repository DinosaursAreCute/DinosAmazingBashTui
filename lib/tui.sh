...
source "${SCRIPT_DIR}/tui_dialog.sh"
source "${SCRIPT_DIR}/tui_text.sh"
source "${SCRIPT_DIR}/tui_widgets.sh"
source "${SCRIPT_DIR}/tui_config.sh"
source "${SCRIPT_DIR}/tui_plugin.sh"
source "${SCRIPT_DIR}/tui_sync.sh"
source "${SCRIPT_DIR}/tui_install.sh"
source "${SCRIPT_DIR}/tui_update.sh"
    ...
    while _tui._next_byte c "$t"; do
        ...
        if [[ "$_TUI_SEQ_BUF" == "O" ]]; then
            ...
            if _tui._next_byte c "$TUI_ESCSEQ_BYTE_TIMEOUT"; then
                case "$c" in [PQRSABCDHFabcd]) _TUI_SEQ_BUF+="$c"; break ;; *) _TUI_PENDING_INPUT="$c$_TUI_PENDING_INPUT"; break ;; esac
            fi
            break
        fi
        case "$c" in
...
tui.run() {
    ...
    _TUI_RUNNING=1
    _TUI_RESIZED=0
    if (( ! _TPL_READY )); then _TPL_READY=1; tui.hook.fire ready; fi
...
tui.stop() { tui.hook.fire quit; _TUI_RUNNING=0; }
