...
_sc_ctl_add() {  # kind row args...  (kind: input|button|label)
    local kind="$1" row="$2"; shift 2
    case "$kind" in
        input)  tui.factory.input  ctl controls "$row" "$1" "" "$2" ;;
        button) tui.factory.button ctl controls "$row" "$1" "$2"; tui.class "$_TUI_FACTORY_LAST_ID" "${3:-info_button}"; tui.align "$_TUI_FACTORY_LAST_ID" fill ;;
        label)  tui.factory.label  ctl controls "$row" "$1"; tui.class "$_TUI_FACTORY_LAST_ID" "${2:-muted_label}" ;;
    esac
}
...
on_box_text()   { _SC_BOX_TEXT="$1"; show_box; }
on_div_label()  { _SC_DIV_LABEL="$1"; show_divider; }
on_gauge_value(){ [[ "$1" =~ ^[0-9]+$ ]] && _SC_GAUGE=$(( $1 > 100 ? 100 : $1 )); show_gauge; }
on_randomize()  { _SC_SERIES=""; _SC_RAND=$RANDOM; _sc_on_resize; }
on_alert_type() {
    local l; l="$(tui.get.label "$1")"; l="${l#\[ }"; l="${l% \]}"
    _SC_ALERT_TYPE="$l"; show_alert
}
...
show_vbar() {
    ...
    local -a v; read -ra v <<< "$(_sc_series 4 100)"
...
show_linechart() {
    ...
    local a b; a="$(_sc_series 14 100)"; b="$(_sc_series 15 100)"
    tui.output "output" "$(linechart -h 12 -w $(( _SC_W - 10 )) -n 0 -m 100 -c 'GREEN,RED' "CPU:${a// /,}" "Mem:${b// /,}")"
}

show_csv() {
    _showcase_set_status "CSV Charts"
    local d="${SCRIPT_DIR}/../examples/csv-charts/data" out=""
    if [[ ! -d "$d" ]]; then
        tui.output "output" "$(alert error "CSV examples not found at $d")"; return
    fi
...
show_fit() {
    ...
    printf -v rule '%*s' $(( w - 2 )) ''
    rule="|${rule// /-}|"
    b="$(tui.get.border output)"; p="$(tui.get.pad output)"
...
sc_theme_pick() {
    local t="${1#btn_sc_th_}" f="${XDG_CONFIG_HOME:-$HOME/.config}/dabt_demo/settings.conf"
    mkdir -p "${f%/*}"
    { grep -v "^theme=" "$f" 2>/dev/null; echo "theme=$t"; } > "$f.tmp" && mv "$f.tmp" "$f"
...
on_fit_pad() {
    local h v; h="$(tui.get inp_fh)"; v="$(tui.get inp_fv)"
    [[ "$h" =~ ^[0-9]$ ]] && tui.pane_pad output "$h" ""
    [[ "$v" =~ ^[0-9]$ ]] && tui.pane_pad output "" "$v"
    tui.relayout; _sc_on_resize
}
on_fit_border() {
    local cur i; cur="$(tui.get.border output)"
    for i in "${!_SC_FIT_BORDERS[@]}"; do
        [[ "${_SC_FIT_BORDERS[i]}" == "$cur" ]] && { cur="${_SC_FIT_BORDERS[(i + 1) % ${#_SC_FIT_BORDERS[@]}]}"; break; }
    done
...
_live_chart() {
    ...
    local n=$(( TUI_PANE_COLS - 12 )); (( n < 10 )) && n=10
    tui.hist.push live_cpu "$TUI_SYS_CPU" "$n"
    tui.hist.push live_mem "$TUI_SYS_MEM_PCT" "$n"
    local c m h=$(( TUI_PANE_ROWS - 4 )); (( h < 4 )) && h=4
    c="$(tui.hist.get live_cpu)"; m="$(tui.hist.get live_mem)"
    [[ "$c" == *" "* ]] || return 0
    _linechart_build -h "$h" -w "$n" -n 0 -m 100 -c 'GREEN,MAGENTA' "CPU:${c// /,}" "Mem:${m// /,}" || return 0
    local out="" l
    for l in "${TR_RESULT[@]}"; do printf -v l '%b' "$l"; out+="$l"$'\n'; done
