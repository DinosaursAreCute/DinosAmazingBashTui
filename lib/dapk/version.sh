...
dapk.version.resolve() {
	...
	[[ "$off" =~ ^[0-9]+$ ]] || {
		DAPK_VERSION_ERROR="build_offset must be a non-negative integer (got '$off')"
		return 1
	}
	if [[ "${GITHUB_REF_TYPE:-}" == tag && "${GITHUB_REF_NAME:-}" == v[0-9]* ]]; then
		...
		v="${CI_COMMIT_TAG#v}"
		src="tag $CI_COMMIT_TAG"
	fi
	if [[ -z "$v" && -r "$dir/VERSION" ]]; then
		read -r v <"$dir/VERSION"
		v="${v//[[:space:]]/}"
		src="VERSION"
	fi
	if [[ -z "$v" && -r "$dir/.dabt.metadata" ]]; then
		while IFS= read -r line || [[ -n "$line" ]]; do
			[[ "$line" =~ ^[[:space:]]*version[[:space:]]*=[[:space:]]*(.*)$ ]] && {
	...
	v="$(_dapk.version.pad "$v")"
	v="${v%%+*}"
	dapk.version.valid "$v" || {
		DAPK_VERSION_ERROR="'$v' (from $src) is not a semantic version"
		return 1
	}
	if [[ -n "$suffix" ]]; then
		[[ "$suffix" =~ ^[0-9A-Za-z.-]+$ ]] || {
			DAPK_VERSION_ERROR="invalid suffix '$suffix'"
			return 1
		}
		v="${v%%-*}-$suffix"
	fi
	n=""
	if [[ "${DABT_BUILD_NUMBER:-}" =~ ^[0-9]+$ ]]; then
		n="$DABT_BUILD_NUMBER" nsrc="DABT_BUILD_NUMBER"
	else
