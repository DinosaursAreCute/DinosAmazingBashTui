...
_showcase_set_status() {
    tui.update "lbl_status" "$1"
}

show_box() {
    _showcase_set_status "Box"
    tui.output "output" "$(box 'DinosAmazingBashTui — pure bash rendering toolkit. No Python. No Node. No ncurses. Just bash, doing things bash was never meant to do.')"
}
...
show_hbar() {
    ...
    out+="$(hbar -m 100 'Q1:60' 'Q2:72' 'Q3:98' 'Q4:85')"$'\n'
    out+=$'\n'
    out+="Language Popularity:"$'\n\n'
    out+="$(hbar 'Bash:100' 'Python:88' 'Rust:65' 'Go:58' 'Lua:30')"
    tui.output "output" "$out"
}
...
show_all() {
    ...
    out+="$(hbar 'Bash:100' 'Fun:99' 'Sanity:12')"$'\n\n'
    out+="$(divider 'BADGES')"$'\n'
