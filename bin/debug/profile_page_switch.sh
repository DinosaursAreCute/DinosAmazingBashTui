...
#   bin/debug/profile_page_switch.sh [-d PAGES_DIR] [-r ROWSxCOLS] PAGE...      (PAGE = file name without .xml)
#   bin/debug/profile_page_switch.sh home settings components debug_input
...
DIR="$(cd "$(dirname "$0")/.." && pwd)"; PAGES_DIR="$DIR/../config/DABT_demo"; ROWS=45; COLS=150
while [[ "$1" == -* ]]; do
    case "$1" in -d) PAGES_DIR="$2"; shift 2 ;; -r) ROWS="${2%x*}"; COLS="${2#*x}"; shift 2 ;; *) break ;; esac
done
...
for p in "$@"; do
    f="$PAGES_DIR/$p.xml"
    tui.goto "$f" >/dev/null 2>&1; tui.goto "$PAGES_DIR/$home.xml" >/dev/null 2>&1        # warm the cache
    t0=${EPOCHREALTIME/[.,]/}; tui.reset_ui >/dev/null 2>&1
    t1=${EPOCHREALTIME/[.,]/}; tui.load_cached "$f" >/dev/null 2>&1
    t2=${EPOCHREALTIME/[.,]/}; tui.render >/dev/null 2>&1
    t3=${EPOCHREALTIME/[.,]/}
