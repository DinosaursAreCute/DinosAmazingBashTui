#!/usr/bin/env bash
# gen_terminal_controls_doc.sh - regenerate docs/api/terminal-controls.md from bin/terminal_controls.sh
# (function name, arguments used, and the trailing "# comment"). Run from anywhere: scripts/gen_terminal_controls_doc.sh
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SRC="$ROOT/bin/terminal_controls.sh"; OUT="$ROOT/docs/api/terminal-controls.md"
{
cat <<'HDR'
